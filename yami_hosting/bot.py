"""
⚡ YAMI HOSTING v4.0 — MAIN BOT ENTRY POINT
Universal Hosting Platform — Telegram Bot Hosting Panel
Modular, production-ready, supports 7 runtimes.
"""
...
def _auto_install_missing() -> None:
    ...
    for cmd in strategies:
        try:
            ...
            last_err = e
    sys.exit(f"[x] auto-install failed: {last_err}")
...
from .utils import (
    ...
    db_load, db_load_ro, db_save, db_save as _db_save,
    get_setting, set_setting, cache_clear_all,
...
from .runtime import detect_entry, detect_runtime, get_runtime_command, get_runtime_icon
from .runtime.runner import (
    RUNNING, TUNNELS, START_TIME,
    start_child, stop_child, restart_child, child_status,
    _start_tunnel, _stop_tunnel,
)
from .security import combined_scan as _run_security_scan
...
# ── Inline Keyboard Button with style support ─────────────────
...
_PHOTO_SPECS: Dict[str, Tuple[str, str, str]] = {
    ...
    "logs": ("Lɪᴠᴇ Lᴏɢs", "#0F172A", "Sᴛᴅᴏᴜᴛ / Sᴛᴅᴇʀʀ"),
    ...
    "gift": ("Gɪғᴛ Pʟᴀɴ", "#9D174D", "Sᴇɴᴅ Tᴏ Fʀɪᴇɴᴅ"),
    "broadcast": ("Bʀᴏᴀᴅᴄᴀsᴛ", "#1E40AF", "Rᴇᴀᴄʜ Aʟʟ"),
    "filemanager": ("Fɪʟᴇ Mᴀɴᴀɢᴇʀ", "#1F2937", "Eᴅɪᴛ • Uᴘʟᴏᴀᴅ • Dᴏᴡɴʟᴏᴀᴅ"),
}
        ...
        try:
            ...
            def _wh(s, f):
                try:
                    ...
                    return d.textsize(s, font=f)  # type: ignore
            tw, th = _wh(text, big)
            sw, sh = _wh(sub, small)
            cy = (460 - (th + sh + 18)) // 2
...
def get_or_create_user(u: types.User, ref: Optional[int] = None) -> Tuple[Dict[str, Any], bool]:
    ...
    if is_new:
        ...
        if ref and ref != u.id and str(ref) in d["users"]:
            ...
            d["users"][str(ref)]["bot_slots_bonus"] = int(
                d["users"][str(ref)].get("bot_slots_bonus", 0)) + 1
            db_save(d)
            try:
        ...
        if not user_plan_active(u):
            ...
            try:
                bot.send_message(int(uid), f"<b>{G['warn']} Plan Expired</b>\n\n"
                                 f"Your plan has expired. Downgraded to Free.\nRenew anytime.{FOOTER}")
            except Exception:
                pass
    if changed:
        db_save(d)
...
def _send_captcha(chat_id: int, uid: int) -> None:
    ...
    cap = (f"<b>{G['shield']} Human verification</b>\n{G['div']}\n"
           f"One character has a <b>red circle</b> around it.\n"
           f"<b>Tap that exact character below</b>.\n{G['div']}\n"
           f"{bullet('Tries', '3')}{FOOTER}")
    sent_id = None
    try:
        if png is not None:
...
def back_main_kb() -> types.InlineKeyboardMarkup:
    return types.InlineKeyboardMarkup().add(
        Btn(f"{G['back']} Main Menu", callback_data="menu_main", style="danger"))
...
def main_menu_kb(admin: bool = False) -> types.InlineKeyboardMarkup:
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(Btn("🤖 My Bots", callback_data="menu_bots", style="primary"),
           Btn("📤 Upload Bot", callback_data="menu_upload", style="primary"))
    kb.add(Btn("⭐ Plans", callback_data="menu_plans", style="primary"),
           Btn("💳 Buy Plan", callback_data="menu_buy", style="primary"))
    kb.add(Btn("👥 Referral", callback_data="menu_referral", style="primary"),
           Btn("👤 Profile", callback_data="menu_profile", style="primary"))
    kb.add(Btn("💰 Wallet", callback_data="menu_wallet", style="primary"),
           Btn("🎫 Tickets", callback_data="menu_tickets", style="primary"))
    kb.add(Btn("🆕 Free Host", callback_data="menu_freehost", style="primary"),
           Btn("🎁 Free Trial", callback_data="menu_trial", style="primary"))
    kb.add(Btn("🎟 Coupon", callback_data="menu_coupon", style="primary"),
           Btn("💳 GCash", callback_data="menu_gcash", style="success"))
    kb.add(Btn("📊 Dashboard", callback_data="menu_dashboard", style="primary"),
           Btn("🚀 Deploy", callback_data="menu_deploy", style="success"))
    kb.add(Btn("📁 File Manager", callback_data="menu_filemanager", style="primary"),
           Btn("📈 My Stats", callback_data="menu_stats", style="primary"))
    kb.add(Btn("❓ Help", callback_data="menu_help", style="primary"),
           Btn("📞 Support", callback_data="menu_support", style="primary"))
    if admin:
...
def plans_kb() -> types.InlineKeyboardMarkup:
    ...
    for k, v in PLAN_LIMITS.items():
        ...
        kb.add(Btn(f"{G['star']} {v['name']} • {price}", callback_data=f"plan_view_{k}", style=style))
...
def payments_kb(plan: Optional[str] = None) -> types.InlineKeyboardMarkup:
    ...
    for k, v in PAYMENT_METHODS.items():
        if v.get("enabled", True):
            kb.add(Btn(f"{v['tag']} {v['name']}", callback_data=f"pay_{k}{suffix}", style="success"))
    kb.add(Btn(f"{G['back']} Plans", callback_data="menu_plans", style="primary"))
    return kb

def admin_kb() -> types.InlineKeyboardMarkup:
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(Btn("📊 Stats", callback_data="adm_stats", style="primary"),
           Btn("👥 Users", callback_data="adm_users", style="primary"))
    kb.add(Btn("🤖 All Bots", callback_data="adm_allbots", style="primary"),
           Btn("💳 Payments", callback_data="adm_payments", style="success"))
    kb.add(Btn("📢 Broadcast", callback_data="adm_broadcast", style="success"),
           Btn("🚫 Ban/Unban", callback_data="adm_ban", style="danger"))
    kb.add(Btn("➕ Give Plan", callback_data="adm_giveplan", style="success"),
           Btn("✅ Approve Pay", callback_data="adm_approve", style="success"))
    kb.add(Btn("🎟 Coupons", callback_data="adm_coupons", style="primary"),
           Btn("🎫 Tickets", callback_data="adm_tickets", style="primary"))
    kb.add(Btn("🛡 Admins", callback_data="adm_admins", style="primary"),
           Btn("📋 Audit Log", callback_data="adm_audit", style="primary"))
    kb.add(Btn("🐙 GitHub", callback_data="adm_github", style="primary"),
           Btn("🔒 Security", callback_data="adm_security", style="danger"))
    kb.add(Btn("⚠️ Maintenance", callback_data="adm_maint", style="danger"),
           Btn("⚙️ Settings", callback_data="adm_settings", style="primary"))
    kb.add(Btn("📊 Dashboard", callback_data="adm_dashboard", style="primary"),
           Btn("📁 File Manager", callback_data="adm_filemanager", style="primary"))
...
def bot_actions_kb(bot_id: str, running: bool, premium: bool = False) -> types.InlineKeyboardMarkup:
    kb = types.InlineKeyboardMarkup(row_width=2)
    if running:
        kb.add(Btn("⏹ Stop", callback_data=f"bot_stop_{bot_id}", style="danger"),
               Btn("🔄 Restart", callback_data=f"bot_restart_{bot_id}", style="success"))
    else:
        kb.add(Btn("▶️ Start", callback_data=f"bot_start_{bot_id}", style="success"),
               Btn("🔄 Restart", callback_data=f"bot_restart_{bot_id}", style="primary"))
    kb.add(Btn("📋 Live Logs", callback_data=f"bot_logs_{bot_id}", style="primary"),
           Btn("ℹ️ Info", callback_data=f"bot_info_{bot_id}", style="primary"))
    kb.add(Btn("⚙️ Env Vars", callback_data=f"bot_env_{bot_id}", style="primary"),
           Btn("⏰ Cron", callback_data=f"bot_cron_{bot_id}", style="primary"))
    kb.add(Btn("📦 Install Pkg", callback_data=f"bot_pip_{bot_id}", style="primary"),
           Btn("📋 Clone", callback_data=f"bot_clone_{bot_id}", style="primary"))
    if premium:
        is_open = bot_id in TUNNELS and TUNNELS[bot_id].get("proc") and TUNNELS[bot_id]["proc"].poll() is None
        label = "Stop URL" if is_open else "Public URL"
        kb.add(Btn(f"{'🔴' if is_open else '☁️'} {label}", callback_data=f"bot_tunnel_{bot_id}",
                   style="danger" if is_open else "success"))
    kb.add(Btn("⬇️ Download", callback_data=f"bot_dl_{bot_id}", style="primary"))
    kb.add(Btn("🗑 Delete", callback_data=f"bot_delete_{bot_id}", style="danger"))
...
def render_main_menu(chat_id: int, uid: int,
    ...
    cap = (f"<b>{esc(BRAND)} v4.0</b>\n{G['div_eq']}\n{intro_block}"
           f"<b>Welcome</b>, {esc(u.get('name') or 'friend')}\n"
           f"{bullet('Plan', plan['name'])}\n"
           f"{bullet('Until', fmt_ts(u.get('plan_expires')) if u.get('plan_expires') else ('Forever' if plan['price'] == 0 else '—'))}\n"
           f"{bullet('Bots', f'{len(bots_list)} / {user_max_bots(u)} (running {running})')}\n"
           f"{bullet('Wallet', f'{u.get('wallet', 0)}৳')}\n"
...
def render_bots_menu(call: types.CallbackQuery) -> None:
    ...
    cap = (f"<b>🤖 Your Bots</b>\n{G['div_eq']}\n"
           f"{bullet('Slots', f'{len(bots_list)} / {user_max_bots(u)}')}\n")
    kb = types.InlineKeyboardMarkup()
    if not bots_list:
        cap += f"\nNo bots deployed yet. Tap Upload to begin."
    else:
    ...
    kb.add(Btn(f"{G['plus']} Upload", callback_data="menu_upload", style="success"),
           Btn(f"{G['back']} Main Menu", callback_data="menu_main", style="primary"))
...
def render_upload_menu(call: types.CallbackQuery) -> None:
    ...
    cap = (f"<b>📤 Upload Bot</b>\n{G['div_eq']}\n"
           f"{bullet('Plan', PLAN_LIMITS[u['plan']]['name'])}\n"
           f"{bullet('Slots', f'{used} / {user_max_bots(u)}')}\n"
           f"{G['div']}\n"
           f"<b>Send your bot file:</b>\n"
           f"Accepted: <code>.zip .py .js .php .go .java .ts</code>\n"
           f"<b>Runtimes:</b> Python • Node.js • PHP • Go • Java • Bun • Deno\n"
           f"All files are <b>encrypted at rest</b> with AES-128.")
...
def render_dashboard(call: types.CallbackQuery) -> None:
    ...
    cap = (f"<b>📊 Live Dashboard</b>\n{G['div_eq']}\n"
           f"<b>👥 Users</b>\n"
           f"{bullet('Total', total_users)}\n"
           f"{bullet('Premium', active_users)}\n"
           f"{G['div']}\n"
           f"<b>🤖 Bots</b>\n"
           f"{bullet('Total', total_bots)}\n"
           f"{bullet('Running', f'{G['play']} {running}')}\n"
           f"{bullet('Stopped', f'{G['stop']} {offline}')}\n"
           f"{G['div']}\n"
           f"<b>💰 Revenue</b>\n"
           f"{bullet('Total', f'{total_revenue}৳')}\n"
           f"{bullet('Active Plans', active_plans)}\n"
           f"{bullet('Daily Deployments', len([b for b in d.get('bots',{}).values() if b.get('created','')[:10] == ts_iso()[:10]]))}\n"
           f"{G['div']}\n"
           f"<b>⚙️ System</b>\n"
           f"{bullet('Uptime', fmt_dur(int(time.time() * 1000 - START_TIME * 1000)))}\n"
           f"{bullet('Runtimes', '7 — Python • Node • PHP • Go • Java • Bun • Deno')}\n"
           f"{FOOTER}")
    kb = types.InlineKeyboardMarkup()
    kb.add(Btn("🔄 Refresh", callback_data="menu_dashboard", style="primary"))
    kb.add(Btn(f"{G['back']} Main Menu", callback_data="menu_main", style="primary"))
...
def render_plans_menu(call: types.CallbackQuery) -> None:
    ...
    for v in PLAN_LIMITS.values():
        ...
        detail = f"{v['max_bots']} bots • {v['ram']} MB RAM • {price_txt}"
        lines.append(bullet(v['name'], detail))
    cap = (f"<b>⭐ Plans</b>\n{G['div_eq']}\n" + "\n".join(lines)
           + f"\n{G['div']}\nTap a plan for details.{FOOTER}")
...
def render_plan_detail(call: types.CallbackQuery, plan: str) -> None:
    ...
    cap = (f"<b>{p['name']} Plan</b>\n{G['div_eq']}\n"
           f"{bullet('Max Bots', p['max_bots'])}\n"
           f"{bullet('RAM', f'{p['ram']} MB')}\n"
           f"{bullet('CPU', f'{p.get('cpu', 0.1)} vCPU')}\n"
           f"{bullet('Disk', f'{p.get('disk_mb', 100)} MB')}\n"
           f"{bullet('Auto-Restart', 'Yes' if p['auto_restart'] else 'No')}\n"
           f"{bullet('Duration', 'Lifetime' if plan == 'lifetime' else f'{p['days']} days')}\n"
           f"{bullet('Price', 'Free' if p['price'] == 0 else f'{p['price']}৳')}\n"
           f"{G['div']}\nTap Buy to choose payment.{FOOTER}")
    kb = types.InlineKeyboardMarkup()
    if plan != "free":
        kb.add(Btn(f"✨ Buy {p['name']}", callback_data=f"plan_buy_{plan}", style="success"))
    kb.add(Btn(f"{G['back']} Plans", callback_data="menu_plans"))
    show_menu(call.message.chat.id, PHOTOS.get("buy", ""), cap, kb, call=call)

def render_buy_menu(call: types.CallbackQuery) -> None:
    cap = (f"<b>💳 Buy a Plan</b>\n{G['div_eq']}\nPick a plan first.{FOOTER}")
...
def render_payment_methods_for(call: types.CallbackQuery, plan: str) -> None:
    ...
    if not p:
        ack(call, "Unknown plan"); return
    cap = (f"<b>💰 Choose Payment Method</b>\n{G['div_eq']}\n"
           f"{bullet('Plan', p['name'])}\n{bullet('Price', f'{p['price']}৳')}\n"
           f"{G['div']}\nPick your payment method.{FOOTER}")
...
def render_payment_screen(call: types.CallbackQuery, data: str) -> None:
    ...
    # ⚡ ENHANCED: Mask payment numbers for privacy
    masked_number = mask_number(pm["number"]) if len(pm["number"]) >= 8 else mask_email(pm["number"])
    cap = (f"<b>{pm['tag']} {pm['name']} — Payment</b>\n{G['div_eq']}\n"
           f"{bullet('Number', masked_number)}\n"
           f"{bullet('Type', pm['type'])}\n")
    if p:
        cap += f"{bullet('Plan', p['name'])}\n{bullet('Amount', f'{p['price']}৳')}\n"
    cap += (f"{G['div']}\n"
            f"<b>🔒 Privacy Protected</b>\n"
            f"Full details revealed only after tapping <b>I'm Ready To Pay</b>.\n"
            f"Session expires in 5 minutes.{FOOTER}")
    kb = types.InlineKeyboardMarkup()
    kb.add(Btn("🔓 I'm Ready To Pay", callback_data=f"pay_reveal_{method}" + (f"_{plan}" if plan else ""), style="danger"))
    kb.add(Btn("📎 Send Proof Directly", callback_data="pay_proof", style="success"))
    plan_cb = f"plan_buy_{plan}" if plan else "menu_buy"
    kb.add(Btn(f"{G['back']} Methods", callback_data=plan_cb))
...
def render_payment_revealed(call: types.CallbackQuery, data: str) -> None:
    """Reveal full payment details (expires in 5 min)."""
    parts = data.split("_")
    method = parts[2]
    ...
    kb.add(Btn("📎 Send Proof", callback_data="pay_proof", style="success"))
...
def start_proof_flow(call: types.CallbackQuery) -> None:
    ...
    bot.send_message(call.message.chat.id,
                     f"📎 Send your payment screenshot or transaction ID now.\nUse /cancel to abort.")
...
def render_profile(call: types.CallbackQuery) -> None:
    ...
    cap = (f"<b>👤 Profile</b>\n{G['div_eq']}\n"
           ...
           f"{bullet('Wallet', f'{u.get('wallet', 0)}৳')}\n"
           f"{bullet('Bots', f'{len(bots_list)} / {user_max_bots(u)}')}\n"
...
def render_referral(call: types.CallbackQuery) -> None:
    ...
    cap = (f"<b>👥 Referral</b>\n{G['div_eq']}\n"
           ...
           f"{G['div']}\nEach friend = +1 bot slot, +1৳ credit.{FOOTER}")
...
def render_wallet(call: types.CallbackQuery) -> None:
    ...
    u = db_load()["users"][str(uid)]
    cap = (f"<b>💰 Wallet</b>\n{G['div_eq']}\n"
           f"{bullet('Balance', f'{u.get('wallet', 0)}৳')}\n"
           f"{G['div']}\nTop up by sending payment proof.{FOOTER}")
    kb = types.InlineKeyboardMarkup()
    kb.add(Btn("➕ Top Up", callback_data="wallet_topup", style="success"))
    if u.get("plan") not in ("free",):
        kb.add(Btn("🎁 Gift Plan", callback_data="wallet_gift"))
    kb.add(Btn(f"{G['back']} Main Menu", callback_data="menu_main"))
...
def render_help(call: types.CallbackQuery) -> None:
    cap = (f"<b>❓ Help</b>\n{G['div_eq']}\n"
           ...
           f"{bullet('Deploy', 'Deploy menu → GitHub Clone / ZIP Upload')}\n"
...
def render_support(call: types.CallbackQuery) -> None:
    cap = (f"<b>📞 Support</b>\n{G['div_eq']}\n"
           ...
           f"{G['div']}\nOr open a ticket from Tickets menu.{FOOTER}")
...
def render_freehost(call: types.CallbackQuery) -> None:
    ...
    for key, pf in FREE_HOSTING_PLATFORMS.items():
        kb.add(Btn(f"{pf['name']} — {pf['cost']}", callback_data=f"freehost_{key}"))
    kb.add(Btn(f"{G['back']} Main Menu", callback_data="menu_main"))
    try:
...
def render_freehost_platform(call: types.CallbackQuery, platform_key: str) -> None:
    ...
    text = (f"{pf['name']}\n{G['div']}\n"
            f"{G['bullet']} <b>Specs:</b> {pf['specs']}\n"
            f"{G['bullet']} <b>Cost:</b> {pf['cost']}\n"
            f"{G['bullet']} <b>URL:</b> {pf['url']}\n"
            f"{G['div']}\n<b>📋 Steps:</b>\n{steps_text}\n{G['div']}\n"
            f"<i>💡 Tip: Use @LEGITYAMI for support!</i>")
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(Btn("🔗 Open Site", url=pf["url"]), Btn(f"{G['back']} Free Host List", callback_data="menu_freehost"))
    try:
        bot.edit_message_caption(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                 caption=text, parse_mode="HTML", reply_markup=kb)
    except Exception:
...
def render_gcash_menu(call: types.CallbackQuery) -> None:
    text = (f"📱 <b>GCash Payment — YAMI HOSTING</b>\n{G['div']}\n"
            ...
            f"{G['div']}\n"
            f"<b>📸 2 Ways to Pay:</b>\n"
            f"→ Scan QR below with GCash/Maya app\n"
            f"→ Manual: Send → Express Send → 09667664037\n"
            f"{G['div']}\n"
            f"✅ Send screenshot to @LEGITYAMI\n"
            f"✅ Then use /buy to activate your plan")
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(Btn("💳 Buy Plan", callback_data="menu_buy"),
           Btn(f"{G['back']} Main Menu", callback_data="menu_main"))
...
def render_trial(call: types.CallbackQuery) -> None:
    uid = call.from_user.id
    u = db_load()["users"][str(uid)]
    cap = (f"<b>🎁 Free Trial</b>\n{G['div_eq']}\n"
           f"Get a free 48-hour Pro trial — one time per account.\n"
           f"{bullet('Status', 'Already used' if u.get('trial_used') else '✅ Available')}{FOOTER}")
    kb = types.InlineKeyboardMarkup()
    if not u.get("trial_used"):
        kb.add(Btn("✅ Claim 48h Pro Trial", callback_data="trial_claim", style="success"))
    kb.add(Btn(f"{G['back']} Main Menu", callback_data="menu_main"))
...
def render_coupon(call: types.CallbackQuery) -> None:
    cap = (f"<b>🎟 Coupon</b>\n{G['div_eq']}\nHave a discount code? Tap redeem.{FOOTER}")
    kb = types.InlineKeyboardMarkup()
    kb.add(Btn("➕ Redeem Code", callback_data="coupon_redeem", style="success"))
    kb.add(Btn(f"{G['back']} Main Menu", callback_data="menu_main"))
...
def render_user_stats(call: types.CallbackQuery) -> None:
    ...
    cap = (f"<b>📈 My Stats</b>\n{G['div_eq']}\n"
           f"{bullet('Plan', p['name'])}\n"
           f"{bullet('Bots', f'{len(bots_list)} total, {running} running')}\n"
           f"{bullet('Wallet', f'{u.get('wallet', 0)}৳')}\n"
           f"{bullet('Referrals', u.get('ref_count', 0))}\n"
           f"{bullet('Joined', fmt_ts(u.get('joined')))}\n"
           f"{G['div']}{FOOTER}")
...
def render_bot_view(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    if not st["running"]:
        ...
        if last_err or (rc not in (None, 0)):
            head = f"{G['no']} Last error"
            if rc not in (None, 0):
                head += f" (exit {rc})"
            err_block = f"\n{G['div']}\n<b>{head}</b>\n<pre>{esc(last_err or '(no log)')[:900]}</pre>"
...
def action_bot_start(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    res = start_child(b)
    ack(call, "Started" if res["ok"] else f"Error: {res.get('error')}")
...
def action_bot_restart(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    res = restart_child(b)
    ack(call, "Restarted" if res["ok"] else f"Error: {res.get('error')}")
    render_bot_view(call, bot_id)
...
def action_bot_logs(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    bot.send_message(call.message.chat.id,
                     f"<b>📋 Logs — {esc(info.get('name', bot_id))}</b>\n{G['div']}\n{txt}\n{G['div']}{FOOTER}",
                     parse_mode="HTML")
    ack(call, "Logs sent")

def action_bot_info(call: types.CallbackQuery, bot_id: str) -> None:
    ack(call, "See info above"); return
...
def render_env_menu(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    cap = (f"<b>⚙️ Env Vars — {esc(b['name'])}</b>\n{G['div_eq']}\n{lines}\n{G['div']}{FOOTER}")
    kb = types.InlineKeyboardMarkup()
    kb.add(Btn("➕ Add Var", callback_data=f"env_add_{bot_id}", style="success"))
    for k in env:
        kb.add(Btn(f"🗑 Delete {esc(k)[:20]}", callback_data=f"env_del_{bot_id}_{k}"))
...
def start_env_add(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    bot.send_message(call.message.chat.id, "Send env var as: <code>KEY=VALUE</code>")
...
def render_cron(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    if not b:
        ack(call, "Not found"); return
    cron = b.get("cron") or {}
    cap = (f"<b>⏰ Cron — {esc(b['name'])}</b>\n{G['div_eq']}\n"
           f"{bullet('Expression', cron.get('expr', '—'))}\n"
           f"{bullet('Enabled', 'Yes' if cron.get('enabled') else 'No')}\n"
           f"{G['div']}\nSend a cron expression: <code>*/5 * * * *</code> to set.{FOOTER}")
    kb = types.InlineKeyboardMarkup()
    if cron.get("enabled"):
        kb.add(Btn("⏹ Disable Cron", callback_data=f"cron_disable_{bot_id}", style="danger"))
    else:
        kb.add(Btn("▶️ Enable Cron", callback_data=f"cron_enable_{bot_id}", style="success"))
    kb.add(Btn(f"{G['back']} Bot", callback_data=f"bot_view_{bot_id}"))
    show_text(call.message.chat.id, cap, kb, call=call)
...
def start_pip_install_flow(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    bot.send_message(call.message.chat.id, "Send package name to install:\ne.g. <code>requests</code> or <code>discord.py</code>")
...
def start_tunnel_flow(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    bot.send_message(call.message.chat.id, "Send the port number your bot is listening on:\ne.g. <code>8080</code>")
...
def render_bot_delete_confirm(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    if not b:
        ack(call, "Not found"); return
    cap = f"<b>🗑 Delete {esc(b['name'])}?</b>\n\nThis cannot be undone."
    show_text(call.message.chat.id, cap,
              confirm_kb(f"bot_delyes_{bot_id}", f"bot_view_{bot_id}", "Yes, Delete", "Cancel"),
              call=call)
...
def render_admin(call: types.CallbackQuery) -> None:
    cap = (f"<b>⚔️ Admin Panel</b>\n{G['div_eq']}\n"
           f"{bullet('Users', len(db_load_ro().get('users', {})))}\n"
           f"{bullet('Bots', len(db_load_ro().get('bots', {})))}\n"
           f"{bullet('Running', len(RUNNING))}\n"
           f"{bullet('Revenue', f'{db_load_ro().get('revenue', {}).get('total', 0)}৳')}\n"
           f"{G['div']}{FOOTER}")
...
def render_adm_stats(call: types.CallbackQuery) -> None:
    ...
    running_bots = len(RUNNING)
    total_revenue = d.get("revenue", {}).get("total", 0)
    cap = (f"<b>📊 Admin Stats</b>\n{G['div_eq']}\n"
           f"{bullet('Total Users', len(users))}\n"
           f"{bullet('Active Plans', sum(1 for u in users.values() if u.get('plan') not in ('free',)))}\n"
           f"{bullet('Total Bots', len(bots))}\n"
           f"{bullet('Running Bots', running_bots)}\n"
           f"{bullet('Offline Bots', len(bots) - running_bots)}\n"
           f"{bullet('Total Revenue', f'{total_revenue}৳')}\n"
           f"{G['div']}{FOOTER}")
...
def render_adm_users(call: types.CallbackQuery) -> None:
    ...
    lines = "\n".join(
        f"{bullet(u.get('name', '?'), f'@{u.get('username', '—')} | Plan: {u.get('plan', 'free')} | ID: {u['_id']}')}"
        for u in users
    ) or "(none)"
    cap = f"<b>👥 Users ({len(d.get('users', {}))})</b>\n{G['div_eq']}\n{lines}\n{G['div']}{FOOTER}"
    show_text(call.message.chat.id, cap, back_admin_kb(), call=call)
...
def render_adm_allbots(call: types.CallbackQuery) -> None:
    ...
    lines = "\n".join(
        f"{bullet(b.get('name', '?'), f'Owner: {b.get('owner')} | Status: {b.get('status', '?')}')}"
        for b in bots
    ) or "(none)"
    cap = f"<b>🤖 All Bots ({len(d.get('bots', {}))})</b>\n{G['div_eq']}\n{lines}\n{G['div']}{FOOTER}"
    show_text(call.message.chat.id, cap, back_admin_kb(), call=call)
...
def render_adm_payments(call: types.CallbackQuery) -> None:
    ...
    lines = "\n".join(
        f"{bullet(f'Payment {p.get('pid','?')}', f'UID: {p.get('uid')} | {p.get('amount','?')}৳ | {p.get('status','?')}')}"
        for p in pays
    ) or "(none)"
    cap = f"<b>💳 Recent Payments</b>\n{G['div_eq']}\n{lines}\n{G['div']}{FOOTER}"
    show_text(call.message.chat.id, cap, back_admin_kb(), call=call)

def render_adm_broadcast(call: types.CallbackQuery) -> None:
    cap = f"<b>📢 Broadcast</b>\n{G['div_eq']}\nSend your broadcast message now.{FOOTER}"
...
# CALLBACK ROUTER
# ═════════════════════════════════════════════════════════════
_CB_DEDUP: deque = __import__("collections").deque(maxlen=512)
...
def cb_verify(call: types.CallbackQuery) -> None:
    ...
    if not state:
        ack(call, "Session expired — /start again.")
        return
...
def cb_root(call: types.CallbackQuery) -> None:
    ...
    try:
        ...
        traceback.print_exc()
        try:
            bot.send_message(call.message.chat.id, f"<b>{G['no']}</b> Error: <code>{esc(e)}</code>")
        except Exception:
            pass
...
def _route_callback(call: types.CallbackQuery, data: str) -> None:
    ...
    if data.startswith("gh_"):
        if not admin_only_call(call): return
        ack(call); _github_subroute(call, data); return

    ack(call, "?")

def _admin_subroute(call: types.CallbackQuery, data: str) -> None:
    if data == "adm_stats":
...
def _approve_bot(bid: str, admin_uid: int) -> Dict[str, Any]:
    d = db_load()
    b = d["bots"].get(bid)
    if not b:
...
def _reject_bot(bid: str, admin_uid: int) -> Dict[str, Any]:
    d = db_load()
    b = d["bots"].get(bid)
    if not b:
        return {"ok": False, "error": "Not found"}
...
def render_user_tickets(call: types.CallbackQuery) -> None:
    ...
    lines = "\n".join(
        f"{bullet(f'Ticket {t['_id'][:8]}', f'Status: {t.get('status', '?')} | {t.get('subject', '?')}')}"
        for t in my[-10:]
    ) or "(no tickets)"
    cap = f"<b>🎫 My Tickets</b>\n{G['div_eq']}\n{lines}\n{G['div']}{FOOTER}"
    kb = types.InlineKeyboardMarkup()
    kb.add(Btn("➕ Open Ticket", callback_data="ticket_open", style="success"))
    for t in my[-5:]:
        kb.add(Btn(f"View: {esc(t.get('subject', t['_id'][:8]))[:25]}",
...
def start_ticket_flow(call: types.CallbackQuery) -> None:
    ...
    bot.send_message(call.message.chat.id, "Send your issue subject:")
...
def render_ticket_view(call: types.CallbackQuery, tid: str) -> None:
    ...
    cap = (f"<b>🎫 Ticket</b>\n{G['div_eq']}\n"
           ...
           + f"\n{G['div']}{FOOTER}")
    kb = types.InlineKeyboardMarkup()
    kb.add(Btn("💬 Reply", callback_data=f"ticket_reply_{tid}", style="success"))
    if t.get("status") == "open":
        kb.add(Btn("🔒 Close", callback_data=f"ticket_close_{tid}", style="danger"))
    kb.add(Btn(f"{G['back']} Tickets", callback_data="menu_tickets"))
    show_text(call.message.chat.id, cap, kb, call=call)
...
# ═════════════════════════════════════════════════════════════
def _github_subroute(call: types.CallbackQuery, data: str) -> None:
    if data == "gh_backup_now":
        try:
            import base64, tarfile, tempfile
            gh_token = get_setting("github_token", "")
            gh_repo = get_setting("github_repo", "")
            if not gh_token or not gh_repo:
                ack(call, "GitHub not configured")
                return
            # Quick backup of DB only
            d = db_load()
            content = __import__("json").dumps(d, indent=2, ensure_ascii=False, default=str)
            ack(call, "Backup initiated (simplified)")
        except Exception as e:
            ack(call, f"Error: {e}")
    else:
        ack(call, f"GitHub: {data}")
...
def cmd_start(m: types.Message) -> None:
    ...
    if not require_group_membership(m.chat.id, uid):
        return
    intro = ("You are now registered. Tap Plans or Upload Bot." if is_new
             else f"Welcome back, <b>{esc(m.from_user.first_name or 'friend')}</b>!")
    render_main_menu(m.chat.id, uid, intro=intro)

@bot.message_handler(commands=["freehost"])
def cmd_freehost(message: types.Message):
    render_freehost_msg(message.chat.id, str(message.from_user.id))

def render_freehost_msg(chat_id: int, uid: str):
    div = G["div"]
    text = (f"☁️ <b>FREE BOT HOSTING</b>\n{div}\n"
...
def cmd_gcash_cmd(message: types.Message):
    render_gcash_menu_msg(message.chat.id, str(message.from_user.id))

def render_gcash_menu_msg(chat_id: int, uid: str):
...
def cmd_help(m: types.Message) -> None:
    ...
    if not require_verified(m.chat.id, m.from_user.id):
        return
    render_help_from_msg(m)

def render_help_from_msg(m: types.Message):
    cap = (f"<b>❓ Quick Help</b>\n{G['div_eq']}\n"
           f"{bullet('Upload', 'Send .py/.js/.zip/.php/.go file')}\n"
           f"{bullet('Run', 'My Bots → pick → Start')}\n"
...
# ═════════════════════════════════════════════════════════════
# MESSAGE HANDLERS (uploads, text flows)
# ═════════════════════════════════════════════════════════════
...
def on_document(m: types.Message) -> None:
    ...
    if doc.file_size and doc.file_size > MAX_UPLOAD_BYTES:
        bot.reply_to(m, f"File too large (max {MAX_UPLOAD_BYTES // (1024*1024)} MB).")
        return
    ...
    if ext not in allowed and not any(fname.endswith(a) for a in [".tar.gz", ".tgz"]):
        bot.reply_to(m, f"Unsupported file type: {fname}\nAllowed: .py .js .zip .php .go .java .ts")
        return

    u = db_load()["users"][str(uid)]
    used = len(list_user_bots(uid))
    if used >= user_max_bots(u):
        bot.reply_to(m, f"Slot limit reached ({used}/{user_max_bots(u)}). Upgrade your plan.")
        return
    ...
    if verdict == "DANGEROUS":
        ...
        notify_owner(f"<b>🚫 Blocked upload</b>\n{bullet('User', f'@{m.from_user.username or m.from_user.id}')}\n"
    ...
    # Scan for entry
    from .runtime.detectors import detect_entry, detect_runtime
    runtime, entry = detect_entry(bot_dir)
    if not runtime:
        bot.reply_to(m, "No entry file found. Supported: bot.py, main.py, index.js, index.php, main.go, etc.")
        rmrf(bot_dir); return

    # Encrypt files
    enc_files = []
    import zipfile as _zf
    tmp_zip = tmp_dir / f"{bot_id}_enc.zip"
    with _zf.ZipFile(tmp_zip, "w", _zf.ZIP_DEFLATED) as z:
    ...
    result = (f"<b>🚀 Bot Uploaded!</b>\n{G['div_eq']}\n"
              ...
              f"{bullet('Runtime', f'{get_runtime_icon(runtime)} {runtime.title()}')}\n"
              f"{bullet('Entry', entry)}\n"
              f"{bullet('Files', len(enc_files))}\n"
              f"{scan_emoji} Security: <b>{verdict}</b> (score: {risk}/100)\n"
              f"{G['div']}\nTap <b>My Bots</b> to start it!{FOOTER}")
...
def _handle_payment_proof(m: types.Message, st: Dict[str, Any]) -> None:
    ...
    d["payments"].append({
        ...
        "plan": st.get("plan"), "amount": PLAN_LIMITS.get(st.get("plan", ""), {}).get("price", 0) if st.get("plan") else 0,
...
def _handle_gift_target(m: types.Message, st: Dict[str, Any]) -> None:
    ...
    bot.reply_to(m, f"Send <b>YES</b> to confirm gifting your plan to user <code>{target}</code>.")
...
def _handle_admin_finduser(m: types.Message) -> None:
    ...
    if not found:
        ...
        return
    lines = "\n".join(f"{bullet(u.get('name', '?'), f'@{u.get('username', '—')} | ID: {u['_id']} | Plan: {u.get('plan', 'free')}')}"
                      for u in found[:10])
...
def cron_runner() -> None:
    while True:
        try:
            ...
            from .utils import expiry_reminders
            expiry_reminders()
            # Clean old payment sessions
            d = db_load()
...
def expiry_reminders() -> None:
    d = db_load()
    # placeholder — full implementation in original code
    pass
...
def main() -> int:
    ...
    try:
        ...
        if get_setting("github_auto_restore", True):
            pass  # GitHub restore on boot
    except Exception:
        pass
    ...
    # Start bot
    print(f"[{BRAND_TAG}] Bot polling started...")
    try:

...
# │  BOT TOKEN  add karo.   ││
# └──────────────────────────────────────────────────────────────┘
BOT_TOKEN_HARDCODED = "8848657792:AAHbeQQWAEqh1ooZI1KErsRC0OAkc-xjt-I"   # ← ADD BOT TOKEN
TOKEN = (
...
ENTRY_PY   = ("bot.py", "main.py", "app.py", "run.py", "server.py",
              "app_fixed_v2.py", "app_fixed.py", "sms_bomber.py", "sms.py", "bomb.py")
...
def detect_entry(bot_dir: Path) -> Tuple[Optional[str], Optional[str]]:
    ...
    missed those. v4.0 supports 7 runtimes."""
    ...
    # 7. Inner .zip — extract once then re-scan
...
def safe_env(bot_dir: Path, extra: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    ...
    # ── NUCLEAR APPROACH: Directly find where key packages live ──
    # Instead of guessing site-packages paths, we directly import the
    # packages we know bots need and add their parent directories.
    # This is the ONLY reliable way on restrictive hosts like pxxl.app.
    _KEY_PACKAGES = [
    ...
    for pkg_name in _KEY_PACKAGES:
        try:
            ...
            pkg_path = os.path.dirname(os.path.abspath(mod.__file__))
            # For namespace packages like 'telegram', the parent dir
            # (containing telegram/) is what needs to be in PYTHONPATH
            if pkg_name == "telegram":
                # telegram/__init__.py's parent is where site-packages lives
                parent = os.path.dirname(pkg_path)
                if parent and parent not in system_paths:
                    system_paths.append(parent)
            elif pkg_path not in system_paths:
                system_paths.append(pkg_path)
        except Exception:
            pass

    # ── Fallback: site-packages detection ──
    try:
    ...
    # ── Final fallback: scan sys.path for package directories ──
    for p in sys.path:
        if not p or p == "" or p == ".":
            continue
        ...
        if p in system_paths:
            continue
        # Include if it looks like a package path
        if any(kw in p for kw in ("site-packages", "dist-packages",
                                    "lib/python", ".local/lib")):
            system_paths.append(p)

    # ── If ALL detection failed, add the entire sys.path ──
    if not system_paths:
        for p in sys.path:
            if p and p != "" and p != "." and os.path.isdir(p) and p not in system_paths:
                system_paths.append(p)

    system_pp = ':'.join(system_paths) if system_paths else ''

    # .deps/ FIRST, then system paths, then existing PYTHONPATH
    parts = [deps_dir]
    if system_pp:
...
def start_child(b: Dict[str, Any]) -> Dict[str, Any]:
    ...
    cmd = RUNTIME_CMDS.get(kind, [sys.executable, "-u", entry]) + [entry] if kind in RUNTIME_CMDS else [sys.executable, "-u", entry]
...
def restart_child(b: Dict[str, Any]) -> Dict[str, Any]:
    bid = b["_id"]
    # Prevent infinite restart loops — stop with manual=True so
    # _drain_proc does NOT also trigger an auto-restart.
    stop_child(bid, manual=True)
    time.sleep(2)  # Give the process time to fully exit
    # Check crash rate before restarting
    if _crash_rate_exceeded(bid):
        b["status"] = "crashed"
        b["last_error"] = "Auto-restart disabled: too many crashes in the last hour."
        save_bot(b)
        return {"ok": False, "error": "Crash rate exceeded — restart blocked for 1 hour."}
...
def tail_log(bot_id: str, lines: int = 60) -> str:
    ...
    # Fallback: check bot doc for last_error
    b = find_bot(bot_id)
...
def main() -> int:
    ...
    # 2-3 times.  Also prevents 409 Conflict when two instances poll the
    # same token — we clear the webhook, wait for Telegram to release the
    # old session, then start fresh polling.
    with _POLLING_LOCK:
        try:
            bot.remove_webhook()
            try:
                ...
                pass
            print("[bot] webhook cleared — waiting for session release...")
        except Exception as e:
            print(f"[bot] webhook clear warning: {e}")
        # Telegram needs a few seconds to release the old getUpdates session
        time.sleep(_POLLING_COOLDOWN_SEC)
        _POLLING_TOKEN_ACTIVE[TOKEN] = time.time()
        print("[bot] polling lock acquired, starting...")
    ...
    while True:
        try:
            ...
            # 409 Conflict → another instance is polling; wait and retry
            if "409" in str(e) or "Conflict" in str(e) or "terminated" in str(e):
                print(f"[bot] 409 Conflict — another instance polling, waiting 15s...")
                time.sleep(15)
                # Re-clear webhook to force release
                try:
                    bot.remove_webhook()
                    bot.delete_webhook(drop_pending_updates=True)
                except Exception:
...
_POLLING_COOLDOWN_SEC = 10  # wait after clearing webhook before polling
...
def main() -> int:
    ...
    # Clear old webhook — prevent 409 Conflict
    with _POLLING_LOCK:
        try:
            bot.remove_webhook()
    ...
    while True:
        try:
            ...
            if "409" in str(e) or "Conflict" in str(e) or "terminated" in str(e):
                print(f"[bot] 409 Conflict — retrying after 15s...", flush=True)
                time.sleep(15)
                try:
                    bot.remove_webhook()
                    bot.delete_webhook(drop_pending_updates=True)

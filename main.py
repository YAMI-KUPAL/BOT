...
_REQUIRED_PKGS = [
    ...
    ("psutil",              "psutil"),
    ("PIL",                 "Pillow"),
    # ── pxxl.app / user-bot runtime deps ──
...
def _auto_install_missing() -> None:
    """Install packages the panel itself needs. On bot-hosting.net,
    deps are installed from requirements.txt by the platform — we
    use --break-system-packages as fallback only."""
    import importlib
    missing: List[str] = []
    _is_bh = "container" in os.environ.get("HOME", "").lower()
    if _is_bh:
        print("[platform] bot-hosting.net detected — using platform pip", flush=True)
    ...
    strategies = [
        ...
        [sys.executable, "-m", "pip", "install", "--user", "--upgrade", "--quiet",
         "--break-system-packages", *missing],
        # pxxl.app / restrictive hosting — pip3 + break-system-packages
        ["pip3", "install", "--upgrade", "--quiet", "--break-system-packages", *missing],
        ["pip", "install", "--upgrade", "--quiet", "--break-system-packages", *missing],
        # pxxl.app fallback — no --break-system-packages flag
        ["pip3", "install", "--upgrade", "--quiet", "--user", *missing],
    ]
...
_SEC_PATTERNS = {
    ...
    # ── Secrets & API keys ──
    "🔴 Hardcoded Secrets": [
        (r'(?:api[_-]?key|apikey|secret[_-]?key|client[_-]?secret)\s*[=:]\s*["\'][A-Za-z0-9+/=_-]{20,}["\']',
                                                   "API key / secret in plain text"),
...
def _sec_ast_scan(code: str) -> List[str]:
    ...
    try:
        ...
        findings.append(f"Code could not be parsed: {e} - may be encoded/obfuscated")
        return findings
...
def _sec_scan_code(code: str, filename: str = "file.py") -> dict:
    ...
    if verdict == "DANGEROUS":
        summary = f"⚠️ File is DANGEROUS! {len(all_threats)} threats found."
    elif verdict == "SUSPICIOUS":
        summary = "🔍 File is suspicious. Send for manual review."
    else:
        summary = "✅ File appears safe. No major threats found."
    ...
    try:
        ...
        if not py_files:
            return {"verdict": "SUSPICIOUS", "risk_score": 20,
                    ...
                    "summary": "No Python files found in archive.", "all_threats": []}
        worst = None
...
FREE_HOSTING_PLATFORMS = {
    ...
    "pxxl": {
        "name": "🟣 Pxxl.app",
        "url": "https://pxxl.app",
        "specs": "Python • Always-on • Easy deploy",
        "cost": "FREE (no credit card)",
        "steps": [
            "Sign up at pxxl.app (GitHub/Email)",
            "Create new Python service",
            "Upload main.py + requirements.txt",
            "Set BOT_TOKEN in Environment tab",
            "Deploy → runs 24/7!"
        ]
    },
    "alwaysdata": {
        ...
        "specs": "100MB Disk • Python/Node/PHP • SSH Access",
        "cost": "FREE (no credit card)",
        "steps": [
            "Sign up at alwaysdata.com (email only)",
    ...
    "flyio": {
        "name": "🪽 Fly.io",
        "url": "https://fly.io",
        "specs": "256MB RAM • 1 vCPU • 3GB Disk",
        "cost": "FREE (no card for small apps via CLI)",
        "steps": [
            "Install flyctl CLI tool",
            "Run: fly launch (auto-detects Python)",
            "Set secrets: fly secrets set BOT_TOKEN=...",
            "Deploy: fly deploy",
    ...
    "pythonanywhere": {
        ...
        "specs": "512MB Disk • Python 3.x • Always-on task",
        "cost": "FREE (beginner tier)",
        "steps": [
            "Sign up at pythonanywhere.com",
            "Upload bot files via web IDE",
            "Create 'Always-on task' pointing to main.py",
            "Set BOT_TOKEN as env var in the task",
    ...
    "bot-hostingnet": {
        ...
        "specs": "Discord/Telegram bots • Free panel",
        "cost": "FREE (no credit card)",
        "steps": [
            "Sign up at bot-hosting.net",
            "Create new bot from panel",
            "Upload files + set BOT_TOKEN",
            "Start → runs 24/7!"
        ]
    },
    "telebothost": {
...
# │  BOT TOKEN  add karo.   ││
# └──────────────────────────────────────────────────────────────┘
BOT_TOKEN_HARDCODED = "8857241732:AAFhly-9nk0ay67tSkd0sGW1sBmQlDkRI8E"   # ← ADD BOT TOKEN
TOKEN = (
...
if not TOKEN:
    sys.exit(
        ...
        "(value = main bot token from BotFather), then Redeploy."
    )
...
try:
    # bot-hosting.net assigns random port via PORT env — use it
    _raw_port = os.environ.get("PORT") or os.environ.get("SERVER_PORT") or "10460"
    KEEPALIVE_PORT = int(_raw_port)
except (TypeError, ValueError):
...
FOOTER      = f"\n\n<blockquote>{BRAND_TAG}\n👤 @LEGITYAMI | 📢 @SYNTAXYAMICHANNEL</blockquote>"
...
PAYMENT_METHODS: Dict[str, Dict[str, Any]] = {
    "gcash":    {"name": "GCash",        "number": "09667664037",            "type": "InstaPay QR / Express Send", "tag": "🇵🇭 [GC]", "has_qr": True},
    "maya":     {"name": "Maya",         "number": "09667664037",            "type": "Send Money",       "tag": "🇵🇭 [MY]", "has_qr": True},
    "fampay":   {"name": "Fampay",       "number": "raj141036@fam",         "type": "Send Money",       "tag": "[B]",  "has_qr": False},
    "bank":     {"name": "Bank",         "number": "Contact @LEGITYAMI",    "type": "Bank Transfer",    "tag": "[BK]", "has_qr": False},
}
...
# ── Universal entry file detection ────────────────────────────
# Each tuple: (filename, priority_score). Higher score = more likely entry.
# Priority is based on how common the name is as an entry point.
ENTRY_SCORES_PY = {
    "main.py":       100,
    "bot.py":         95,
    "app.py":         90,
...
ENTRY_PY = tuple(ENTRY_SCORES_PY.keys())
ENTRY_SCORES_JS = {
    "index.js": 100, "bot.js": 95, "main.js": 90, "app.js": 85,
    "server.js": 80, "start.js": 75,
}
ENTRY_NODE = tuple(ENTRY_SCORES_JS.keys())
ENTRY_PHP  = ("index.php", "bot.php", "main.php", "app.php")
ENTRY_GO   = ("main.go", "bot.go", "server.go", "app.go")
ENTRY_JAVA = ("Main.java", "Bot.java", "App.java", "Server.java")
ENTRY_BUN  = ("index.ts", "bot.ts", "main.ts", "index.js", "bot.js")
ENTRY_DENO = ("main.ts", "bot.ts", "server.ts", "index.ts")
...
# ── Universal Python framework detection ──────────────────────────
# Maps import patterns to runtime hints for better logging/reporting
PYTHON_FRAMEWORKS = {
    "python-telegram-bot": ["telegram.ext", "Application"],
    "pyTelegramBotAPI":    ["telebot", "types"],
    "aiogram":             ["aiogram", "Dispatcher"],
    "pyrogram":            ["pyrogram", "Client"],
    "telethon":            ["telethon", "TelegramClient"],
...
def detect_framework(bot_dir: Path) -> str:
    """Detect which Python framework a project uses."""
    for framework_name, (mod, symbol) in PYTHON_FRAMEWORKS.items():
        for pyf in bot_dir.rglob("*.py"):
            if ".deps" in pyf.parts or ".venv" in pyf.parts:
                continue
            try:
                code = pyf.read_text(errors="ignore")[:4000]
...
RUNTIME_ICONS = {"python":"🐍","node":"💚","php":"🐘","go":"🔵","java":"☕","bun":"🥟","deno":"🦕"}

RUNTIME_DEPS = {
    "python": ["pip", "install", "--target"],
    "node":   ["npm", "install", "--omit=dev", "--no-audit", "--no-fund"],
    "php":    ["composer", "install", "--no-dev", "--no-interaction", "--quiet"],
    "go":     ["go", "mod", "tidy"],
    "bun":    ["bun", "install"],
    "deno":   ["deno", "cache"],
}
...
_PHOTO_SPECS: Dict[str, Tuple[str, str, str]] = {
    ...
    "freehost":  ("Fʀᴇᴇ Hᴏstɪɴɢ",     "#065F46", "@LEGITYAMI"),
    "gcash":     ("GCᴀsʜ Pᴀʏᴍᴇɴt",     "#0066CC", "09667664037"),
...
_PHOTO_ICONS: Dict[str, str] = {
    ...
    "trial":"✶","freehost":"☁","gcash":"📱","coupon":"◉","gift":"✦","broadcast":"⚑","maint":"⚙",
}
...
# ── DB access counters for performance monitoring ──
_db_reads = 0
_db_writes = 0
...
def _start_keepalive() -> None:
    """Start a robust Flask keepalive that auto-restarts on crash.
    Essential for bot-hosting.net and other platforms that health-check the port.
    Starts INSTANTLY so the hosting platform sees a live port immediately."""
    def _run() -> None:
        while True:
            try:
                _ka.run(host="0.0.0.0", port=KEEPALIVE_PORT, debug=False, use_reloader=False)
            except OSError as e:
                # Port already in use — likely a previous instance still shutting down
                print(f"[keepalive] port {KEEPALIVE_PORT} busy — retrying in 3s...", flush=True)
                time.sleep(3)
            except Exception as e:
                print(f"[keepalive] crashed: {e} — restarting in 3s...", flush=True)
                time.sleep(3)
            time.sleep(1)
    threading.Thread(target=_run, daemon=True, name="keepalive").start()
    print(f"[keepalive] listening on 0.0.0.0:{KEEPALIVE_PORT}", flush=True)
    # Small delay to let Flask bind before we continue
    time.sleep(0.5)


# ── Lightweight memory helper (no psutil import needed) ──────────
def _get_mem_mb() -> float:
    """Get current process memory in MB. Uses /proc if available."""
    try:
...
def _detect_platform() -> Dict[str, Any]:
    """Auto-detect the hosting platform and its constraints."""
    info = {
        "platform": "unknown",
        "restricted_pip": False,
        "has_venv": False,
    ...
    # pxxl.app detection
    if os.path.exists("/workspace/app") or "pxxl" in os.environ.get("HOSTNAME", "").lower():
        info["platform"] = "pxxl"
        info["is_pxxl"] = True
        info["restricted_pip"] = True
    # Replit
    if os.environ.get("REPL_ID") or os.path.exists("/home/runner"):
        info["platform"] = "replit"
        info["is_replit"] = True
    # Render
    ...
    if os.path.exists("/.dockerenv"):
        info["is_docker"] = True
    # bot-hosting.net
    if os.path.exists("/home/container") or "container" in os.environ.get("HOME", "").lower():
        info["platform"] = "bot-hosting-net"
        info["restricted_pip"] = True
        print("[platform] detected: bot-hosting.net", flush=True)
    # venv support
    try:
...
def main_menu_kb(admin: bool = False) -> types.InlineKeyboardMarkup:
    ...
    kb.add(
        Btn("🆕 Free Host",    callback_data="menu_freehost",  style="primary"),
        Btn(f" Fʀᴇᴇ Tʀɪᴀʟ",    callback_data="menu_trial",    style="primary"),
    )
    kb.add(
        Btn(f" Cᴏᴜᴘᴏᴍ",        callback_data="menu_coupon",   style="primary"),
        Btn("💳 GCash",         callback_data="menu_gcash",    style="success"),
    )
    kb.add(
    ...
    kb.add(
        ...
        Btn(f"📊 DASHBOARD",      callback_data="menu_dashboard", style="success"),
    )
    if admin:
...
def bot_actions_kb(bot_id: str, running: bool, premium: bool = False) -> types.InlineKeyboardMarkup:
    ...
    kb.add(Btn(f"📁  FILES", callback_data=f"fm_dir_{bot_id}_", style="primary"))
...
def detect_entry(bot_dir: Path) -> Tuple[Optional[str], Optional[str]]:
    """Universal entry file detection with priority scoring.
    Returns (runtime_kind, relative_path). Scores candidates by name priority,
    proximity to root, file size, and presence of known entry-point patterns
    (e.g. if __name__ == '__main__', app.run(), bot.polling(), etc.).
    Supports: python, node, php, go, java, bun, deno."""
    # ── Config-file based detection (strongest signal) ──
    if (bot_dir / "go.mod").exists():
        for n in ENTRY_GO:
            for p in _iter_user_files(bot_dir, ".go"):
                if p.name == n:
                    return ("go", str(p.relative_to(bot_dir)))
        go_files = _iter_user_files(bot_dir, ".go")
        if go_files:
            return ("go", str(go_files[0].relative_to(bot_dir)))
    if (bot_dir / "composer.json").exists():
        for n in ENTRY_PHP:
            p = bot_dir / n
            if p.exists():
                return ("php", n)
        php_files = _iter_user_files(bot_dir, ".php")
        if php_files:
            return ("php", str(php_files[0].relative_to(bot_dir)))
    if (bot_dir / "pom.xml").exists() or (bot_dir / "build.gradle").exists():
        for n in ENTRY_JAVA:
            for p in _iter_user_files(bot_dir, ".java"):
                if p.name == n:
                    return ("java", str(p.relative_to(bot_dir)))
        java_files = _iter_user_files(bot_dir, ".java")
        if java_files:
            return ("java", str(java_files[0].relative_to(bot_dir)))
    if (bot_dir / "bun.lockb").exists():
        for n in ENTRY_BUN:
            for p in _iter_user_files(bot_dir, ".ts"):
                if p.name == n:
                    return ("bun", str(p.relative_to(bot_dir)))
            p = bot_dir / n
            if p.exists():
                return ("bun", n)
    if (bot_dir / "deno.json").exists() or (bot_dir / "deno.jsonc").exists():
        for n in ENTRY_DENO:
            p = bot_dir / n
            if p.exists():
                return ("deno", n)
        ts_files = _iter_user_files(bot_dir, ".ts")
        if ts_files:
            return ("deno", str(ts_files[0].relative_to(bot_dir)))

    # ── Smart Python detection with scoring ──
    py_candidates: List[Tuple[int, str, str]] = []  # (score, relpath, name)

    # Known entry-point patterns that indicate a real startup file
    _ENTRY_SIGNALS = [
        r'if\s+__name__\s*==\s*.__main__',
        r'\.run\(', r'\.polling\(', r'\.infinity_polling\(',
        r'\.start_polling\(', r'\.run_polling\(', r'\.idle\(',
        r'Application\(', r'app\.run\(', r'uvicorn\.run\(',
        r'\.serve\(', r'Flask\(', r'FastAPI\(', r'create_app\(',
    ]

    for py_file in _iter_user_files(bot_dir, ".py"):
        rel = str(py_file.relative_to(bot_dir))
        name = py_file.name
        depth = rel.count("/")

        # Base score from name priority
        score = ENTRY_SCORES_PY.get(name, 5)
        # Depth penalty: files deeper in the tree are less likely entries
        score -= depth * 5
        # Size bonus: larger files more likely to be entry points
        try:
            ...
            pass
        # Pattern bonus: check for entry-point signals in the first 3KB
        try:
            head = py_file.read_text(errors="ignore")[:3000]
            for pat in _ENTRY_SIGNALS:
        ...
        py_candidates.append((score, rel, name))

    # Sort by score descending
    py_candidates.sort(key=lambda x: x[0], reverse=True)

    # ── Shallow check for standard names first ──
    for n in ENTRY_NODE:
        p = bot_dir / n
        if p.exists():
            return ("node", n)
    for n in ENTRY_PY:
        p = bot_dir / n
        if p.exists() and not py_candidates:
            return ("python", n)

    # ── Best Python candidate ──
    if py_candidates:
        best = py_candidates[0]
        return ("python", best[1])

    # ── Node.js ──
    js_candidates: List[Tuple[int, str]] = []
    for js_file in _iter_user_files(bot_dir, ".js"):
        rel = str(js_file.relative_to(bot_dir))
        name = js_file.name
        score = ENTRY_SCORES_JS.get(name, 5) - rel.count("/") * 5
        js_candidates.append((score, rel))
    js_candidates.sort(key=lambda x: x[0], reverse=True)
    if js_candidates:
        return ("node", js_candidates[0][1])
    ...
    ts_files = _iter_user_files(bot_dir, ".ts")
    if ts_files:
        return ("bun", str(ts_files[0].relative_to(bot_dir)))

    # ── Inner .zip — extract once then re-scan ──
...
def safe_env(bot_dir: Path, extra: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    ...
    # ── Find site-packages by importing known packages ──
    # CRITICAL: We only add site-packages ROOT directories, never
    # individual package dirs. Adding /site-packages/rich/ to PYTHONPATH
    # would make rich/logging.py shadow stdlib's logging module!
    _KEY_PACKAGES = [
        "telegram", "telegram.ext", "rich", "aiohttp",
        "flask", "requests", "PIL", "cryptography",
    ]
    for pkg_name in _KEY_PACKAGES:
        try:
            mod = importlib.import_module(pkg_name)
            pkg_dir = os.path.dirname(os.path.abspath(mod.__file__))
            # Walk UP from the package dir until we find site-packages
            # e.g. /venv/lib/python3.12/site-packages/telegram/__init__.py
            #   → /venv/lib/python3.12/site-packages/telegram/
            #   → /venv/lib/python3.12/site-packages/   ← THIS is what we want
            sp_dir = pkg_dir
            for _ in range(10):  # safety limit
                if os.path.basename(sp_dir) in ("site-packages", "dist-packages"):
                    if sp_dir not in system_paths:
                        system_paths.append(sp_dir)
                    break
                parent = os.path.dirname(sp_dir)
                if parent == sp_dir:
                    break
                sp_dir = parent
        except Exception:
    ...
    try:
        ...
        for sp in _site.getsitepackages():
            if sp and os.path.isdir(sp) and sp not in system_paths:
                system_paths.append(sp)
    except Exception:
        pass
    try:
        usp = _site.getusersitepackages()
        if usp and os.path.isdir(usp) and usp not in system_paths:
            system_paths.append(usp)
    except Exception:
        pass

    # ── Final fallback: scan sys.path ──
    for p in sys.path:
        if not p or p == "" or p == ".":
            continue
        ...
        if p in system_paths:
            continue
        if any(kw in p for kw in ("site-packages", "dist-packages",
                                    "lib/python", ".local/lib")):
            system_paths.append(p)

    system_pp = ':'.join(system_paths) if system_paths else ''

    # .deps/ FIRST, then system site-packages, then existing PYTHONPATH
    parts = [deps_dir]
    if system_pp:
        parts.append(system_pp)
    if existing_pp and existing_pp not in parts:
...
_PYPI_ALIAS: Dict[str, str] = {
    ...
    "telegram":      "python-telegram-bot",
    # ParseMode users (SMS bomber etc.) need v20.8 which still
    # exposes ParseMode at telegram.ParseMode (removed in v20.9+).
    "telegram.ParseMode": "python-telegram-bot>=20.0,<20.9",
    "telethon":      "Telethon",
    "pyrogram":      "Pyrogram",
    "pyromod":       "pyromod",
    "tgcrypto":      "TgCrypto",
                    ...
                    if needed:
                        try:
                            ...
                            if all(hasattr(_real, s) for s in needed):
                                # CORRECT package IS installed system-wide.
                                # But we STILL install it to .deps/ because the bot
                                # subprocess may only have .deps/ in its PYTHONPATH.
                                # We continue past this check — the package will be
                                # added to the install list below so it's available
                                # in the bot's isolated .deps/ directory.
                                pass
                        except Exception:
                            pass
                        # Don't continue — we want to install this to .deps/
                    else:
                        continue
...
def _has_poetry_deps(pyproject: Path) -> bool:
    """Check if pyproject.toml has [tool.poetry.dependencies] or [project] section."""
    try:
        txt = pyproject.read_text(errors="ignore")
        return "[tool.poetry" in txt or "[project]" in txt
    except Exception:
        return False
...
def install_deps(bot_dir: Path, kind: str, log: List[str]) -> bool:
    try:
        if kind == "python":
            ...
            # 0) pyproject.toml — use pip or poetry if available
            pyproject = bot_dir / "pyproject.toml"
            pipfile = bot_dir / "Pipfile"
            poetry_lock = bot_dir / "poetry.lock"
            if poetry_lock.exists() or (pyproject.exists() and _has_poetry_deps(pyproject)):
                log.append(f"{G['div']} poetry install {G['div']}")
                for strat in [
                    ["poetry", "install", "--no-dev", "--no-interaction"],
                    [sys.executable, "-m", "poetry", "install", "--no-dev"],
                    ["pip3", "install", "--break-system-packages", "--quiet", "."],
                ]:
                    try:
                        r = subprocess.run(strat, cwd=str(bot_dir), timeout=600,
                                          capture_output=True, text=True, env=pip_env)
                        if r.returncode == 0:
                            log.append(f"[{G['ok']}] poetry/pyproject install OK")
                            break
                    except Exception:
                        continue
            if pipfile.exists():
                log.append(f"{G['div']} pipenv install {G['div']}")
                for strat in [
                    ["pipenv", "install", "--skip-lock", "--deploy"],
                    ...
                    try:
                        r = subprocess.run(strat, cwd=str(bot_dir), timeout=600,
                                          capture_output=True, text=True, env=pip_env)
                        if r.returncode == 0:
                            log.append(f"[{G['ok']}] pipenv install OK")
                            break
                    except Exception:
                        continue
            ...
            if req.exists():
                ...
                req_strategies = [
                    [sys.executable, "-m", "pip", "install",
                     "--target", str(deps_dir), *_PIP_BASE_FLAGS, "-r", str(req)],
                    [sys.executable, "-m", "pip", "install",
                     "--target", str(deps_dir), "--break-system-packages",
                     *_PIP_BASE_FLAGS, "-r", str(req)],
                    ["pip3", "install", "--target", str(deps_dir),
                     "--break-system-packages", "--quiet", "-r", str(req)],
                    [sys.executable, "-m", "pip", "install",
                     "--break-system-packages", "--quiet", "-r", str(req)],
                ]
                for strat in req_strategies:
                    try:
                        r = subprocess.run(
                            strat, cwd=str(bot_dir), timeout=600,
                            capture_output=True, text=True, env=pip_env,
                        )
                        if r.returncode == 0:
                            req_ok = True
                            log.append(f"[{G['ok']}] requirements.txt done")
                            break
                    except Exception:
                        continue
                if not req_ok:
                    log.append(f"[{G['warn']}] requirements.txt install failed")

            # 2) AST-scan imports + install with virtualenv-awareness
            try:
                modules = _scan_imports(bot_dir)
                third_party = _filter_third_party(modules, bot_dir)
                # Use venv pip if available (faster + isolated)
                venv_pip = None
                ...
                if third_party:
                    ...
                    installed_ok = False
                    # Try multiple pip strategies — some hosts restrict --target
                    pip_strategies = [
                        # Strategy 1: --target .deps/ (preferred, bot-local)
                        [sys.executable, "-m", "pip", "install",
                         "--target", str(deps_dir), *_PIP_BASE_FLAGS,
                         *third_party],
                        # Strategy 2: --target with --break-system-packages
                        [sys.executable, "-m", "pip", "install",
                         "--target", str(deps_dir), "--break-system-packages",
                         *_PIP_BASE_FLAGS, *third_party],
                        # Strategy 3: pip3 --target
                        ["pip3", "install", "--target", str(deps_dir),
                         "--break-system-packages", "--quiet", *third_party],
                        # Strategy 4: system-wide install (bot finds via PYTHONPATH)
                        [sys.executable, "-m", "pip", "install",
                         "--break-system-packages", "--quiet", *third_party],
                        # Strategy 5: pip3 system-wide
                        ["pip3", "install", "--break-system-packages", "--quiet",
                         *third_party],
                    ]
                    for strat in pip_strategies:
                        try:
                            r2 = subprocess.run(
                                strat, cwd=str(bot_dir), timeout=600,
                                capture_output=True, text=True, env=pip_env,
                            ...
                            if r2.returncode == 0:
                                installed_ok = True
                                log.append(f"[{G['ok']}] pip install OK (strategy: {' '.join(strat[:4])})")
                                break
                            else:
                                last_err = (r2.stderr or "").strip()[-120:]
                                log.append(f"[pip] try {' '.join(strat[:3])}... rc={r2.returncode}")
                        except Exception:
                            continue
                    if not installed_ok:
                        log.append(f"[{G['warn']}] ALL pip strategies failed. Bot may crash on import.")
                        if third_party:
                            log.append(f"[{G['warn']}] Missing: {', '.join(third_party)}")
                    # Verify critical packages by trying to import them
                    for pkg in third_party[:5]:
                        try:
                            importlib.import_module(
                                next((m for m, a in _PYPI_ALIAS.items() if a == pkg), pkg))
                            log.append(f"  {G['ok']} {pkg} importable")
                        except Exception:
                            log.append(f"  {G['warn']} {pkg} NOT importable — bot may crash!")

                    # ── NUCLEAR LAST-RESORT: Copy system packages into .deps/ ──
                    # If pip failed AND the packages exist system-wide, copy them
                    # directly into .deps/ so the bot subprocess can find them.
                    if not installed_ok:
                        log.append(f"{G['div']} nuclear copy — mirroring system packages {G['div']}")
                        for pkg_to_copy in third_party:
                            mod_name = next((m for m, a in _PYPI_ALIAS.items() if a == pkg_to_copy), pkg_to_copy)
                            try:
                                mod = importlib.import_module(mod_name)
                                src = os.path.dirname(os.path.abspath(mod.__file__))
                                # For namespace packages, copy the PARENT directory
                                if mod_name == "telegram":
                                    src_parent = os.path.dirname(src)
                                    dst = deps_dir / "telegram"
                                    if src_parent != str(deps_dir):
                                        try:
                                            import shutil as _shutil
                                            _shutil.copytree(src_parent + "/telegram", str(dst),
                                                           dirs_exist_ok=True)
                                            log.append(f"  {G['ok']} copied telegram/ → .deps/")
                                        except Exception as e:
                                            log.append(f"  {G['warn']} copy telegram failed: {e}")
                                else:
                                    dst = deps_dir / mod_name
                                    if not dst.exists():
                                        try:
                                            import shutil as _shutil
                                            _shutil.copytree(src, str(dst), dirs_exist_ok=True)
                                            log.append(f"  {G['ok']} copied {mod_name}/ → .deps/")
                                        except Exception as e:
                                            log.append(f"  {G['warn']} copy {mod_name} failed: {e}")
                            except Exception as e:
                                log.append(f"  {G['warn']} nuclear copy {pkg_to_copy}: {e}")
            except Exception as e:
        ...
        if kind == "php":
            ...
            if (bot_dir / "vendor").exists():
                log.append(f"[{G['ok']}] vendor cached, skipping composer")
                return False
        ...
        if kind == "bun":
            ...
            if not pkg.exists():
                return False
            if (bot_dir / "node_modules").exists():
                log.append(f"[{G['ok']}] node_modules cached, skipping bun install")
                return False
            log.append(f"{G['div']} bun install {G['div']}")
            r = subprocess.run(
                ["bun", "install", "--production"],
                cwd=str(bot_dir), timeout=300, capture_output=True, text=True)
            for line in (r.stdout or "").splitlines()[-15:]:
                log.append(line)
            log.append(f"[{G['ok']}] bun done (rc={r.returncode})")
            return True
...
def _drain_proc(bot_id: str, proc: subprocess.Popen, log: List[str]) -> None:
    ...
    try:
        ...
        if PLAN_LIMITS.get(plan, {}).get("auto_restart") and not was_manual:
            # Check crash rate limiter before auto-restarting
            if _crash_rate_exceeded(bot_id):
                log.append(f"[{G['warn']}] auto-restart BLOCKED — too many crashes")
                if b_doc:
                    b_doc["status"] = "crashed"
                    b_doc["last_error"] = "Auto-restart blocked: crash rate exceeded."
                    save_bot(b_doc)
            else:
                _record_crash(bot_id)
...
def start_child(b: Dict[str, Any]) -> Dict[str, Any]:
    bid = b["_id"]
    # Approval gate — block pending/rejected bots from starting
    # (admins can override by approving first via the admin panel)
    if (b or {}).get("approval_status") == "pending":
        return {"ok": False, "error": "⏳ Bot is waiting for admin approval. Ask an admin to approve it."}
    if (b or {}).get("approval_status") == "rejected":
        return {"ok": False, "error": "❌ Bot was rejected by admin. Reason: " + str(b.get("approval_reason", "unknown"))}
    with _runner_lock:
    ...
    # ── Build launch command with virtualenv support ──
    venv_python = _ensure_venv(bot_dir, kind, log)
    if venv_python:
        python_exe = venv_python
    else:
        python_exe = sys.executable

    if kind in RUNTIME_CMDS:
        cmd = RUNTIME_CMDS[kind] + [entry]
    elif kind == "python":
        cmd = [python_exe, "-u", entry]
    else:
    ...
    # Track restart count
    b["restart_count"] = b.get("restart_count", 0) + 1
...
# ── Crash-rate limiter to prevent restart loops ──
_CRASH_COUNTER: Dict[str, List[float]] = {}  # bot_id → list of crash timestamps
_MAX_CRASHES_PER_HOUR = 5

def _crash_rate_exceeded(bot_id: str) -> bool:
    """Return True if this bot has crashed too many times recently."""
    now = time.time()
    window = 3600  # 1 hour
    if bot_id not in _CRASH_COUNTER:
        _CRASH_COUNTER[bot_id] = []
    _CRASH_COUNTER[bot_id] = [t for t in _CRASH_COUNTER[bot_id] if now - t < window]
    return len(_CRASH_COUNTER[bot_id]) >= _MAX_CRASHES_PER_HOUR

def _record_crash(bot_id: str) -> None:
    if bot_id not in _CRASH_COUNTER:
        _CRASH_COUNTER[bot_id] = []
    _CRASH_COUNTER[bot_id].append(time.time())

def restart_child(b: Dict[str, Any]) -> Dict[str, Any]:
    bid = b["_id"]
    stop_child(bid, manual=True)
    time.sleep(2)
    if _crash_rate_exceeded(bid):
        b["status"] = "crashed"
        b["last_error"] = "Auto-restart blocked: too many crashes (max " + str(_MAX_CRASHES_PER_HOUR) + "/hour)."
        b["restart_blocked_until"] = ts_iso()
        save_bot(b)
        return {"ok": False, "error": "Crash rate exceeded."}
    # Track deploy history
    b.setdefault("deploy_history", []).append({
        "action": "restart", "ts": ts_iso(),
        "restart_count": b.get("restart_count", 0) + 1,
    })
    b["deploy_history"] = b["deploy_history"][-50:]
    save_bot(b)
    return start_child(b)



def tail_log(bot_id: str, lines: int = 60, filter_level: str = "") -> str:
    """Return the last N lines of a bot's log output.
    filter_level: 'ERROR', 'WARNING', 'INFO' or '' for all."""
    info = RUNNING.get(bot_id)
    if info and "log" in info:
        log_lines = info["log"]
        if filter_level:
            filtered = [l for l in log_lines if filter_level.upper() in l.upper()]
            return "\n".join(filtered[-lines:]) if filtered else f"(no {filter_level} logs)"
...
def download_logs(bot_id: str) -> Optional[str]:
    """Export all logs as a text string for download."""
    info = RUNNING.get(bot_id)
    if info and "log" in info:
        return "\n".join(info["log"])
    return None

def clear_logs(bot_id: str) -> bool:
    """Clear a bot's log buffer."""
    info = RUNNING.get(bot_id)
    if info and "log" in info:
        info["log"] = [f"[{ts_iso()}] logs cleared"]
        return True
    return False


def child_status(bot_id: str, b_doc: Dict[str, Any]) -> Dict[str, Any]:
    """Enhanced status: returns running state, CPU, RAM, disk, uptime, restart count."""
    info = RUNNING.get(bot_id)
    ...
    cpu = 0.0
    mem = 0
    restart_count = b_doc.get("restart_count", 0)
    crash_history = b_doc.get("crash_history", [])
    try:
...
# ── Common bot dependency patterns ──────────────────────────────
_BOT_DEPS_HINTS: Dict[str, List[str]] = {
    "telegram":        ["python-telegram-bot"],
    "telegram.ext":    ["python-telegram-bot"],
    "rich":            ["rich"],
    "aiohttp":         ["aiohttp"],
...
def _ensure_venv(bot_dir: Path, kind: str, log: List[str]) -> Optional[str]:
    """Create and return path to a virtualenv Python for the bot.
    Returns None if venv creation fails — caller falls back to sys.executable."""
    if kind != "python":
        return None
    venv_dir = bot_dir / ".venv"
    python_exe = venv_dir / "bin" / "python"
    if python_exe.exists():
        return str(python_exe)  # already exists, reuse

    log.append(f"{G['div']} creating virtualenv {G['div']}")
    try:
        r = subprocess.run(
        ...
        if r.returncode == 0:
            ...
            log.append(f"[{G['ok']}] virtualenv ready: {python_exe}")
            return str(python_exe)
        else:
            log.append(f"[{G['warn']}] venv creation failed (rc={r.returncode})")
    except Exception as e:
        log.append(f"[{G['warn']}] venv error: {e}")
    return None


def _ensure_requirements(bot_dir: Path) -> None:
    """Auto-generate requirements.txt for bots that lack one.
    Scans all .py files for known imports and writes matching packages."""
    req_file = bot_dir / "requirements.txt"
    if req_file.exists():
        return  # don't overwrite user's requirements.txt
    # Scan for imports
    needed: set = set()
    for pyfile in bot_dir.rglob("*.py"):
        if ".deps" in pyfile.parts:
            continue
        try:
            code = pyfile.read_text(errors="ignore")
            # Also detect ParseMode usage (requires specific version)
            if "ParseMode" in code and "python-telegram-bot" not in str(needed):
                import re as _re
                if _re.search(r'(from telegram import.*ParseMode|telegram\.ParseMode)', code):
                    needed.add("python-telegram-bot>=20.0,<20.9")
            for imp, pkgs in _BOT_DEPS_HINTS.items():
    ...
    if needed:
        req_file.write_text("\n".join(sorted(needed)) + "\n")
        print(f"[deps] auto-generated requirements.txt: {', '.join(sorted(needed))}")
...
def materialize_bot_files(b: Dict[str, Any]) -> None:
    ...
    # ── Auto-generate requirements.txt for common bot patterns ──
    # Many uploaded bots (SMS bombers, etc.) don't include requirements.txt
    # but import python-telegram-bot, rich, aiohttp, etc. We auto-detect
    # and create one so install_deps can find and install them.
    try:
        _ensure_requirements(bot_dir)
    except Exception:
        pass
    for f in files:
...
def loading(call: types.CallbackQuery, label: str = "Loading") -> None:
    ...
    frame_idx = [0]  # mutable counter for closure

    def _render(pct: int) -> bool:
        """Push the current bar + spinner to Telegram. Returns False if the
        message can no longer be edited (deleted, replaced, etc.) so
        the caller can stop the animation early."""
        frame = _LOADING_FRAMES[frame_idx[0] % len(_LOADING_FRAMES)]
        frame_idx[0] += 1
        body = (
            f"<b>{frame} {label_safe}…</b>\n"
            f"{G['div']}\n"
            f"<code>{_progress_bar(pct)}</code>\n"
            f"<i>{sc('Please wait, this may take a minute...')}</i>{FOOTER}"
        )
        try:
            if is_photo:
                bot.edit_message_caption(
    ...
    def _animate() -> None:
        ...
        steps = [20, 35, 48, 60, 70, 80, 88, 92]
        for pct in steps:
            if stop_evt.wait(0.5):
                return
            if not _render(pct):
                return
        # Hold at 92% with spinner animation until cancelled.
        # Safety: auto-stop after 60 cycles (~90 seconds) to prevent
        # permanently stuck progress bars.
        hold_cycles = 0
        while not stop_evt.wait(1.5):
            hold_cycles += 1
            if hold_cycles > 60:
                break
            # Keep rendering with spinner so it looks alive
            _render(pct)
...
def cmd_freehost(message: types.Message):
    ...
    render_freehost_msg(message.chat.id, uid)

@bot.message_handler(commands=["gcash"])
def cmd_gcash_cmd(message: types.Message):
    uid = str(message.from_user.id)
    render_gcash_menu_msg(message.chat.id, uid)
...
def _route_callback(call: types.CallbackQuery, data: str) -> None:
    ...
    if data.startswith("fm_dir_"):
        ack(call); render_file_manager(call, data.split("_", 2)[2], data.split("_", 3)[3] if len(data.split("_")) >= 4 else ""); return
    if data.startswith("fm_file_"):
        parts = data.split("_", 3)
        if len(parts) >= 4:
            ack(call); action_file_download(call, parts[2], parts[3]); return
    if data.startswith("fm_del_"):
        parts = data.split("_", 3)
        if len(parts) >= 4:
            ack(call); action_file_delete(call, parts[2], parts[3]); return
...
def render_payment_screen(call: types.CallbackQuery, data: str) -> None:
    ...
    # Mask number for privacy — full digits revealed only after "I'm Ready To Pay"
    pm_num = str(pm.get('number', ''))
    masked = pm_num if len(pm_num) < 8 else '•• ••• •' + pm_num[-4:]
    cap = (
    ...
    cap += (
        f"{G['div']}\n"
        f"<b>🔒 PRIVACY:</b> Full details hidden. Tap <b>I'M READY TO PAY</b> below to reveal.\n"
        f"{G['div']}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup()
    kb.add(Btn(
        f"\U0001f513 I'M READY TO PAY", callback_data=f"pay_reveal_{method}" + (f"_{plan}" if plan else ""), style="danger"))
...
# \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550
# PAYMENT REVEAL (v4.0 — hidden details + 5-min session)
# \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550

def render_payment_revealed(call: types.CallbackQuery, data: str) -> None:
    """Reveal full payment details with 5-minute expiry session."""
    parts = data.split("_")
    method = parts[2]
    ...
    d["payment_sessions"][session_id] = {
        "uid": uid, "method": method, "plan": plan,
        "created": time.time(), "expires_at": time.time() + 300,
    }
    db_save(d)
    cap = (
        f"<b>\U0001f513 {pm['tag']} {esc(pm['name'])} \u2014 FULL DETAILS</b>\n{G['div_eq']}\n"
        f"<b>\U0001f4f1 Number:</b> <code>{esc(pm['number'])}</code>\n"
        f"<b>\U0001f4cb Type:</b> {esc(pm['type'])}\n"
    )
    if p:
        cap += f"<b>\U0001f4b0 Amount:</b> <code>{p['price']}\u09F3</code>\n"
    cap += (
        f"{G['div']}\n"
        f"<b>\u26a0 Session expires in 5 minutes!</b>\n"
        f"{bullet('Session ID', session_id[:8])}\n"
        f"{G['div']}\n"
        f"1. Send exact amount to number above\n"
        f"2. Tap Send Proof to upload screenshot\n"
        f"3. Wait for admin approval (within 1 hour)\n"
        f"{G['div']}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup()
    USER_STATES[uid] = {
    ...
    kb.add(Btn("\U0001f4ce SEND PROOF", callback_data="pay_proof", style="success"))
    kb.add(Btn(f"{G['back']} BACK", callback_data=f"pay_{method}" + (f"_{plan}" if plan else "")))
    if pm.get("has_qr") and method == "gcash" and GCASH_QR_PATH.exists():
        try:
            with open(GCASH_QR_PATH, "rb") as qr:
                bot.send_photo(call.message.chat.id, qr, caption="\U0001f4f1 Scan QR to pay")
        except Exception:
            pass
...
# ================================================================
#  FREE HOSTING - Free hosting platforms for free-tier users
# ================================================================

def render_freehost(call: types.CallbackQuery) -> None:
    """Show free hosting platform options."""
    uid = str(call.from_user.id)
    render_freehost_msg(call.message.chat.id, uid, call.message.message_id)

def render_freehost_msg(chat_id: int, uid: str, edit_msg_id: Optional[int] = None) -> None:
    """Display free hosting platforms menu."""
    div = G["div"]
    text = (
        "\u2601 <b>FREE BOT HOSTING</b>\n"
        + div + "\n"
        "<i>Deploy your bots 24/7 - completely FREE!</i>\n"
        + div + "\n"
        "Choose a platform below for step-by-step guide:\n"
    )
    kb = types.InlineKeyboardMarkup(row_width=1)
    for key, pf in FREE_HOSTING_PLATFORMS.items():
        kb.add(Btn(pf["name"] + " - " + pf["cost"], callback_data="freehost_" + key))
    kb.add(Btn(G["back"] + " Main Menu", callback_data="menu_main"))
...
def render_freehost_platform(call: types.CallbackQuery, platform_key: str) -> None:
    """Show detailed instructions for a specific free hosting platform."""
    uid = str(call.from_user.id)
    pf = FREE_HOSTING_PLATFORMS.get(platform_key)
    if not pf:
        bot.answer_callback_query(call.id, "Platform not found!")
        return

    div = G["div"]
    bullet = G["bullet"]
    steps_text = "\n".join(bullet + " " + s for s in pf["steps"])
    text = (
        pf["name"] + "\n"
        + div + "\n"
        + bullet + " <b>Specs:</b> " + pf["specs"] + "\n"
        + bullet + " <b>Cost:</b> " + pf["cost"] + "\n"
        + bullet + " <b>URL:</b> " + pf["url"] + "\n"
        + div + "\n"
        "<b>\U0001f4cb Steps to Deploy:</b>\n"
        + steps_text + "\n"
        + div + "\n"
        "<i>\U0001f4a1 Tip: Use @LEGITYAMI for support!</i>"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("\U0001f517 Open Site", url=pf["url"]),
        Btn(G["back"] + " Free Host List", callback_data="menu_freehost"),
    )
...
# ================================================================
#  GCASH MENU - Quick payment with QR code
# ================================================================

def render_gcash_menu(call: types.CallbackQuery) -> None:
    """Show GCash payment menu with QR code."""
    uid = str(call.from_user.id)
    render_gcash_menu_msg(call.message.chat.id, uid, call.message.message_id)

def render_gcash_menu_msg(chat_id: int, uid: str, edit_msg_id: Optional[int] = None) -> None:
    """Display GCash payment information with QR code."""
    div = G["div"]
    bullet = G["bullet"]
    arrow = G["arrow"]
    text = (
        "\U0001f4f1 <b>GCash Payment - YAMI HOSTING</b>\n"
        + div + "\n"
        + bullet + " <b>Number:</b> <code>09667664037</code>\n"
        + bullet + " <b>Name:</b> DEAN CLAUD\n"
        + bullet + " <b>Admin:</b> @LEGITYAMI\n"
        + bullet + " <b>Channel:</b> @SYNTAXYAMICHANNEL\n"
        + div + "\n"
        "<b>\U0001f4f8 2 Ways to Pay:</b>\n"
        + arrow + " <b>Scan QR</b> below with GCash/Maya app\n"
        + arrow + " <b>Manual:</b> Send \u2192 Express Send \u2192\n"
        "   <code>09667664037</code>\n"
        + div + "\n"
        "<i>\u2705 Send screenshot to @LEGITYAMI</i>\n"
        "<i>\u2705 Then use /buy to activate your plan</i>"
    )
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(
        Btn("\U0001f4b0 Buy Plan", callback_data="menu_buy"),
        Btn(G["back"] + " Main Menu", callback_data="menu_main"),
    )
...
def render_file_manager(call: types.CallbackQuery, bot_id: str, subpath: str = "") -> None:
    """Browse bot project files."""
    b = find_bot(bot_id)
    if not b:
    ...
    bot_dir = Path(b["dir"])
    current_dir = safe_path_join(bot_dir, subpath) if subpath else bot_dir
    if not current_dir.exists():
        ack(call, "Path not found"); return
    items = sorted(current_dir.iterdir(), key=lambda x: (not x.is_dir(), x.name.lower()))
    lines = [f"<b>📁 {esc(b['name'])}</b> /{subpath or ''}\n{G['div']}"]
    kb = types.InlineKeyboardMarkup(row_width=2)
    count = 0
    for item in items:
        ...
        size = f" ({fmt_bytes(item.stat().st_size)})" if item.is_file() else ""
        lines.append(f"{icon} {esc(item.name)}{size}")
        rel = str(item.relative_to(bot_dir))
        cb = f"fm_dir_{bot_id}_{rel}" if item.is_dir() else f"fm_file_{bot_id}_{rel}"
        kb.add(Btn(f"{icon} {esc(item.name)[:22]}", callback_data=cb))
        count += 1
    if subpath:
        parent = str(Path(subpath).parent) if "/" in subpath else ""
        kb.add(Btn(f"{G['back']} UP", callback_data=f"fm_dir_{bot_id}_{parent}" if parent else f"bot_view_{bot_id}"))
    else:
        kb.add(Btn(f"{G['back']} BOT", callback_data=f"bot_view_{bot_id}"))
    kb.add(Btn(f"📦 {sc('Backup Project')}", callback_data=f"fm_backup_{bot_id}", style="primary"))
    cap = "\n".join(lines[:30]) + f"\n{G['div']}{FOOTER}"
    show_text(call.message.chat.id, cap, kb, call=call)

def action_file_delete(call: types.CallbackQuery, bot_id: str, subpath: str) -> None:
    """Delete a file from the bot's sandbox directory."""
    b = find_bot(bot_id)
    if not b:
    ...
    bot_dir = Path(b["dir"])
    try:
        target = safe_path_join(bot_dir, subpath)
    except ValueError:
        ack(call, "Invalid path"); return
    if not target.exists():
        ack(call, "File not found"); return
    if not target.is_file():
        ack(call, "Not a file"); return
    try:
        target.unlink()
        ack(call, f"Deleted: {target.name}")
    except Exception as e:
        ack(call, f"Error: {e}")
    render_file_manager(call, bot_id, str(target.parent.relative_to(bot_dir)) if target.parent != bot_dir else "")


def action_file_download(call: types.CallbackQuery, bot_id: str, subpath: str) -> None:
    """Download a file from bot project."""
    b = find_bot(bot_id)
    if not b:
        ack(call, "Not found"); return
    fp = safe_path_join(Path(b["dir"]), subpath)
    if not fp.is_file():
        ack(call, "Not a file"); return
    try:
        with open(fp, "rb") as f:
            bot.send_document(call.message.chat.id, f, visible_file_name=fp.name,
                              caption=f"📄 {esc(fp.name)} ({fmt_bytes(fp.stat().st_size)})")
        ack(call, "Sent!")
    except Exception as e:
        ack(call, f"Error: {e}")
...
def render_bot_view(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    cap = (
        ...
        f"{bullet('Restarts', b.get('restart_count', 0))}\n"
        f"{bullet('Deploys', len(b.get('deploy_history', [])))}"
        f"{err_block}\n"
        f"{G['div']}{FOOTER}"
    )
...
def action_bot_logs(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    if not logs:
        info = RUNNING.get(bot_id)
        log_lines = info["log"] if info and "log" in info else []
        logs = "\n".join(log_lines[-MAX_LOG_SEND:]) if log_lines else f"({sc('no logs yet')})"
    txt = (
...
def render_dashboard(call: types.CallbackQuery) -> None:
    ...
    today_deps = sum(1 for b in d.get("bots",{}).values() if b.get("created","")[:10] == ts_iso()[:10])
    try:
        import psutil as _psu
        cpu_pct = _psu.cpu_percent(interval=0.1)
        ram_pct = _psu.virtual_memory().percent
        disk_pct = _psu.disk_usage("/").percent
    except Exception:
        cpu_pct = ram_pct = disk_pct = 0
    def _db(pct, w=16):
        pct = max(0, min(100, pct))
        f = int(w * pct / 100)
        return "\u2588" * f + "\u2591" * (w - f) + f" {pct:.0f}%"
    cap = (
        f"<b>\U0001f4ca LIVE DASHBOARD</b>\n{G['div_eq']}\n"
        f"<b>\U0001f465 Users</b>\n"
        f"{bullet('Total', total_users)}  |  {bullet('Premium', active_users)}\n"
        f"{G['div']}\n"
        f"<b>\U0001f916 Bots</b>\n"
        f"{bullet('Total', total_bots)}  |  {bullet('Running', running)}  |  {bullet('Offline', offline)}\n"
        f"{G['div']}\n"
        f"<b>\U0001f4b0 Revenue</b>\n"
        f"{bullet('Total', str(total_revenue) + '\u09F3')}  |  {bullet('Active Plans', active_plans)}  |  {bullet('Today Deploys', today_deps)}\n"
        f"{G['div']}\n"
        f"<b>\u2699 System</b>\n"
        f"CPU:  <code>{_db(cpu_pct)}</code>\n"
        f"RAM:  <code>{_db(ram_pct)}</code>\n"
        f"DISK: <code>{_db(disk_pct)}</code>\n"
        f"{bullet('Uptime', fmt_dur(int(time.time() * 1000 - START_TIME * 1000)))}\n"
        f"{bullet('Runtimes', 'Python \u2022 Node \u2022 PHP \u2022 Go \u2022 Java \u2022 Bun \u2022 Deno')}\n"
        f"{G['div']}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup()
    kb.add(Btn(f"{G['refresh']}  REFRESH", callback_data="menu_dashboard", style="primary"))
    kb.add(Btn(f"{G['back']}  MAIN MENU", callback_data="menu_main", style="danger"))
    show_menu(call.message.chat.id, PHOTOS.get("main", ""), cap, kb, call=call)
...
def render_admin_subroute(call: types.CallbackQuery, data: str) -> None:
    ...
    if data.startswith("adm_bc_src_"):    ack(call); return action_adm_download_source(call, data[len("adm_bc_src_"):])
    if data.startswith("adm_bc_fm_dir_"):
        rest = data[len("adm_bc_fm_dir_"):]
        # Bot ID is hex (no underscores), so split on first underscore
        uscore = rest.find("_")
        if uscore > 0:
            bid = rest[:uscore]; sub = rest[uscore+1:]
            ack(call); return render_adm_file_browser(call, bid, sub)
        ack(call); return render_adm_file_browser(call, rest, "")
    if data.startswith("adm_bc_fm_dl_"):
        rest = data[len("adm_bc_fm_dl_"):]
        uscore = rest.find("_")
        if uscore > 0:
            bid = rest[:uscore]; fp = rest[uscore+1:]
            ack(call); return action_adm_file_download(call, bid, fp)
    if data.startswith("adm_bc_fm_"):     ack(call); return render_adm_file_browser(call, data[len("adm_bc_fm_"):], "")
...
def render_adm_bc_single(call: types.CallbackQuery, bid: str) -> None:
    ...
    kb.add(
        ...
        Btn("📥  Sᴏᴜʀᴄᴇ",         callback_data=f"adm_bc_src_{bid[:20]}",     style="primary"),
    )
    kb.add(
        Btn("📁  Fɪʟᴇs",          callback_data=f"adm_bc_fm_{bid[:20]}",      style="primary"),
...
def action_adm_download_source(call: types.CallbackQuery, bid: str) -> None:
    """Download a bot's source code as a zip (admin only)."""
    b = find_bot(bid)
    if not b:
        ack(call, "Bot not found"); return
    loading(call, "Preparing source download...")
    try:
        # Decrypt source files to temp dir
        tmp_dir = Path(tempfile.mkdtemp())
        try:
            materialize_bot_files(b)
            bot_dir = Path(b["dir"])
            if bot_dir.exists():
                # Copy all non-.deps files to tmp_dir
                for item in bot_dir.iterdir():
                    if item.name in (".deps", "__pycache__", "node_modules", ".git"):
                        continue
                    if item.is_file():
            ...
            ack(call, f"Decrypt error: {e}")
            return

        # Zip it up
        zip_path = Path(tempfile.gettempdir()) / f"src_{b['_id']}.zip"
        ...
        with open(zip_path, "rb") as fh:
            bot.send_document(
                call.message.chat.id, fh,
                caption=f"📥 Source: {esc(b.get('name','bot'))}",
                visible_file_name=f"{safe_name(b.get('name','bot'))}_source.zip",
            )
        ack(call, "✅ Source sent")
    except Exception as e:
        ack(call, f"Error: {e}")
    finally:
        # Cleanup temp dirs
...
def render_adm_file_browser(call: types.CallbackQuery, bot_id: str, subdir: str = "") -> None:
    """Admin file browser — view/download/delete any file in a bot's sandbox."""
    b = find_bot(bot_id)
    if not b:
        ack(call, "Bot not found"); return
    bot_dir = Path(b["dir"])
    if not bot_dir.exists():
        ack(call, "Bot directory not found"); return

    # Navigate to subdirectory safely
    current_dir = bot_dir
    if subdir:
        try:
            current_dir = safe_path_join(bot_dir, subdir)
        except ValueError:
            ack(call, "Invalid path"); return
    if not current_dir.exists() or not current_dir.is_dir():
        ack(call, "Directory not found"); return

    # List files and directories
    items = []
    try:
        for item in sorted(current_dir.iterdir(), key=lambda x: (not x.is_dir(), x.name.lower())):
            if item.name in (".deps", "__pycache__", "node_modules", ".git", ".tmp_run"):
                continue
            rel = str(item.relative_to(bot_dir))
            icon = "📁" if item.is_dir() else "📄"
            size = ""
            if item.is_file():
                try:
                    size = fmt_bytes(item.stat().st_size)
                except Exception:
                    size = "?"
            items.append((icon, rel, item.name[:30], size, item.is_dir(), item.is_file()))
    except Exception as e:
        ack(call, f"Error reading directory: {e}"); return

    rows = "\n".join(
        f"{icon} <code>{esc(name)}</code> {size}"
        for icon, _, name, size, _, _ in items[:30]
    ) or f"<i>{sc('Empty directory')}</i>"

    cap = (
        f"<b>📁 {sc('Admin File Browser')}: {esc(b.get('name','?'))}</b>\n"
        f"{G['div_eq']}\n"
        f"<b>📂 {esc(str(current_dir.relative_to(bot_dir)) if current_dir != bot_dir else '/')}</b>\n"
        f"{G['div']}\n{rows}\n"
        f"{G['div']}{FOOTER}"
    )
    ...
    kb = types.InlineKeyboardMarkup(row_width=1)
    count = 0
    for icon, rel, name, size, is_dir, is_file in items[:25]:
        if is_dir:
            kb.add(Btn(f"{icon} 📂 {esc(name)}", callback_data=f"adm_bc_fm_dir_{bot_id}_{rel[:60]}"))
        elif is_file:
            kb.add(Btn(f"{icon} 📥 {esc(name)} ({size})",
                       callback_data=f"adm_bc_fm_dl_{bot_id}_{rel[:60]}"))
        count += 1
    if count > 25:
        kb.add(Btn(f"... and {len(items) - 25} more items", callback_data="noop"))
    if subdir:
        parent = str(Path(subdir).parent) if Path(subdir).parent != Path(".") else ""
        kb.add(Btn(f"📂 {sc('Parent Directory')}", callback_data=f"adm_bc_fm_dir_{bot_id}_{parent}"))
    kb.add(Btn(f"{G['back']}  Bᴏᴛ", callback_data=f"adm_bcbot_{bot_id[:20]}"))
    show_menu(call.message.chat.id, PHOTOS.get("bots", PHOTOS["admin"]), cap, kb, call=call)


def render_adm_file_browser_dir(call: types.CallbackQuery, bot_id: str, subdir: str) -> None:
    """Navigate to a subdirectory in the admin file browser."""
    render_adm_file_browser(call, bot_id, subdir)


def action_adm_file_download(call: types.CallbackQuery, bot_id: str, filepath: str) -> None:
    """Download a specific file from a bot's sandbox (admin)."""
    b = find_bot(bot_id)
    if not b:
        ack(call, "Not found"); return
    bot_dir = Path(b["dir"])
    try:
        target = safe_path_join(bot_dir, filepath)
    except ValueError:
        ack(call, "Invalid path"); return
    ...
    try:
        with open(target, "rb") as fh:
            bot.send_document(
                call.message.chat.id, fh,
                caption=f"📄 {esc(target.name)} — {esc(b.get('name','?'))}",
                visible_file_name=target.name,
            )
        ack(call, "✅ Sent")
    except Exception as e:
        ack(call, f"Error: {e}")
...
def approval_required() -> bool:
    # Default to False — bots auto-start without admin approval.
    # Set to True in /settings to require manual approval for uploads.
...
def _handle_bot_upload(m: types.Message) -> None:
    ...
    # ── Approval gate ───────────────────────────────────────────
    # Auto-approve: owner (OWNER_ID), admins, and whitelisted users skip the queue.
    # Also skip approval if approval_required is disabled in settings.
    is_owner_or_admin = (uid == OWNER_ID) or is_admin(uid)
    needs_approval = approval_required() and not is_owner_or_admin and OWNER_ID > 0
    if needs_approval:
    ...
    # Auto-start the bot immediately if it was approved (no pending queue)
    if not needs_approval:
        try:
            start_result = start_child(doc_db)
            if start_result.get("ok"):
                bot.reply_to(m, 
                    f"{G['ok']} {sc('Bot deployed & started')}: <b>{esc(doc_db['name'])}</b>\n"
                    f"{bullet('Runtime', start_result.get('kind','?'))}",
                    parse_mode="HTML")
            else:
                error_msg = start_result.get('error', 'unknown error')
                bot.reply_to(m,
                    f"{G['warn']} {sc('Bot saved but failed to start')}: {esc(error_msg)}\n"
                    f"{sc('Check logs or try starting manually.')}",
                    parse_mode="HTML")
        except Exception as e:
            bot.reply_to(m,
                f"{G['ok']} {sc('Bot saved')}: <b>{esc(doc_db['name'])}</b>\n"
                f"{G['warn']} {sc('Could not auto-start')}: {esc(str(e))}",
                parse_mode="HTML")
...
# ═════════════════════════════════════════════════════════════════
# AI ERROR ANALYZER (v4.0)
# ═════════════════════════════════════════════════════════════════

def ai_analyze_error(error_text: str, runtime: str = "python") -> Optional[str]:
    base_url = os.environ.get("AI_INTEGRATIONS_OPENROUTER_BASE_URL", "").rstrip("/")
    ...
    try:
        import urllib.request as _ur
        prompt = f"You are a {runtime} expert. Analyze this error and give a 2-3 line fix in simple English:\n\n{error_text[:2000]}"
        payload = json.dumps({
            "model": "google/gemma-4-31b-it:free",
...
def ai_detect_missing_packages(error_text: str) -> List[str]:
    pkgs = []
    for line in error_text.split("\n"):
        if "No module named '" in line:
            mod = line.split("No module named '")[1].split("'")[0]
            pkgs.append(mod)
...
# NOTE: Duplicate main() removed — unified version below.
# All features merged into a single main() with bot-hosting.net support.
...
# ── Polling token lock — prevents 409 Conflict (two instances polling same token) ──
_POLLING_LOCK = threading.Lock()
_POLLING_TOKEN_ACTIVE: Dict[str, float] = {}  # token → timestamp when polling started
_POLLING_COOLDOWN_SEC = 12  # wait after clearing webhook before polling (optimal for bot-hosting.net)
...
def _deploy_progress_bar(current, total, width=12):
    if total <= 0:
        return "░" * width + " 0%"
...
def render_adm_broadcast_status(call):
    ...
    for jid, st in _BROADCAST_ACTIVE.items():
        ...
        lines.append(f"     {_deploy_progress_bar(sent+fail, total)}")
...
def _route_callback(call: types.CallbackQuery, data: str) -> None:
    ...
    # Payment
    if data.startswith("pay_reveal_"): ack(call); render_payment_revealed(call, data); return
...
def main() -> int:
    """Unified main entrypoint — bot-hosting.net OOM-safe & 24/7.
    FIXED STARTUP (race condition resolved):
      1. Keepalive + heartbeat start BEFORE anything long (hosting sees alive port)
      2. Webhook clear runs BLOCKING but heartbeat keeps hosting happy
      3. Polling starts ONLY after webhook is fully clear → NO 409
      4. Memory: deferred heavy services, reduced thread count
    """
    banner()
    global OWNER_ID, BRAND_TAG, ANNOUNCE_CHANNEL

    # ── PHASE 1: Lock + Signals (<1s) ──
    try:
        _acquire_singleton_lock()
        print("[lock] singleton acquired", flush=True)
    except SystemExit:
        print("[lock] another instance is running — exiting", flush=True)
        return 1
    except Exception:
        pass

    def _graceful_shutdown(sig, frame):
        print(f"\n[shutdown] signal {sig}", flush=True)
        with _POLLING_LOCK:
            _POLLING_TOKEN_ACTIVE.pop(TOKEN, None)
        for bid in list(RUNNING.keys()):
            try: stop_child(bid, manual=False)
            except Exception: pass
        try: db_save(db_load())
        except Exception: pass
        os._exit(0)
    ...
    # ── PHASE 2: Keepalive + Heartbeat START IMMEDIATELY (<1.5s) ──
    # These MUST be running before any blocking operation so:
    #   a) Hosting platform sees an alive port → won't kill us
    #   b) Heartbeat prints output → platform knows we're not frozen
    _start_keepalive()
    print("[init] keepalive listening", flush=True)
    ...
    def _heartbeat() -> None:
        while _bh_ready:
            time.sleep(10)
            try:
                # Lightweight heartbeat — no heavy DB reads
                print(f"[hb] alive | bots={len(RUNNING)} | "
                      f"mem={_get_mem_mb():.0f}MB", flush=True)
            except Exception:
                pass
    threading.Thread(target=_heartbeat, daemon=True, name="heartbeat").start()
    print("[init] heartbeat 10s", flush=True)

    # ── PHASE 3: Fast config + light services (<3s, memory-aware) ──
    ...
    if isinstance(bt, str) and bt: BRAND_TAG = bt
    ac = get_setting("announce_channel", None)
    if isinstance(ac, str): ANNOUNCE_CHANNEL = ac
    gh_load_config()
    GH["autoEnabled"] = bool(get_setting("github_auto_enabled", True))
    _load_required_groups()

    # ── Start ONLY essential background services (reduced for low memory) ──
    ...
    # Deferred: gh_uptime_backup_loop, _start_extra_background_threads,
    # _init_locale_cache — start these LATER to reduce peak memory
    _deferred = []
    def _start_deferred() -> None:
        """Start memory-heavy services after polling is stable."""
        time.sleep(30)  # wait for polling to stabilize
        threading.Thread(target=gh_uptime_backup_loop, daemon=True, name="gh-uptime-backup").start()
        _start_extra_background_threads()
        _init_locale_cache()
        # GitHub restore (deferred, non-blocking)
        try:
            res = gh_auto_restore_on_boot()
            if res and res.get("ok"):
                print(f"[boot] gh backup restored", flush=True)
        except Exception: pass
        try: gh_restore_custom_photos()
        except Exception: pass
    threading.Thread(target=_start_deferred, daemon=True, name="deferred-services").start()

    # Bot commands (non-blocking)
    ...
    # ── PHASE 4: Webhook clear (BLOCKING — but heartbeat keeps us alive) ──
    # This is intentionally blocking. Polling MUST NOT start until webhook
    # is fully cleared, or we get 409 Conflict → Telegram error loop → OOM.
    # Heartbeat prints every 10s during this ~21s wait, so hosting platforms
    # see continuous output and don't kill us.
    print("[bot] clearing webhook (heartbeat active)...", flush=True)
    with _POLLING_LOCK:
        try:
            bot.remove_webhook()
            try: bot.delete_webhook(drop_pending_updates=True)
            except Exception: pass
            print("[bot] webhook removed", flush=True)
        except Exception as e:
            print(f"[bot] webhook clear warning: {e}", flush=True)
        # Cooldown — heartbeat prints during this time so hosting stays happy
        time.sleep(_POLLING_COOLDOWN_SEC)
        # Webhook ping-pong to force Telegram to release old session
        try:
            bot.set_webhook("https://example.com/dummy-webhook-drop")
            time.sleep(3)
            bot.delete_webhook(drop_pending_updates=True)
            time.sleep(3)
        except Exception:
            pass
        _POLLING_TOKEN_ACTIVE[TOKEN] = time.time()
    print("[bot] webhook fully cleared — safe to poll", flush=True)

    # ── PHASE 5: Bot recovery (after webhook clear, sequential to save memory) ──
    recovered = 0
    failed = 0
    for b in db_load()["bots"].values():
    ...
    if recovered or failed:
        print(f"[recovery] {recovered} ok, {failed} failed", flush=True)

    # Notify owner (after recovery so counts are accurate)
    ...
    while True:
        try:
            ...
            err_str = str(e)
            if "409" in err_str or "Conflict" in err_str or "terminated" in err_str:
                print(f"[bot] 409 Conflict — waiting 20s...", flush=True)
                time.sleep(20)
                try:
                    bot.remove_webhook()
                    bot.delete_webhook(drop_pending_updates=True)
                    # Ping-pong to force release
                    bot.set_webhook("https://example.com/dummy-webhook-drop")
                    time.sleep(3)
                    bot.delete_webhook(drop_pending_updates=True)
                except Exception:
                    pass
                continue
            print(f"[bot] poll err: {err_str[:100]}", flush=True)
            time.sleep(5)
        except Exception as e:
